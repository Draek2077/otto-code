...
```bash
otto plugin add /absolute/path/to/otto/plugin-examples/linear
otto plugin ls linear
```

Use `otto plugin reload linear` after source edits. Changing the API key requires restarting the
