...
created_at: "2026-08-08T06:18:01.720Z"
updated_at: "2026-08-08T06:19:52.551Z"
---
...
- time: "2026-08-08T06:18:01.720Z"
  kind: "decision"
  summary: "Knowledge page created."
- time: "2026-08-08T06:18:01.720Z"
  kind: "evidence"
  summary: "Migrated from `projects/user-mode/user-mode.md` and the legacy `projects/README.md` ledger. Legacy status: Charter. Ledger summary: The same agentic engine **re-skinned around deliverables instead of a repo**, so a non-coder can generate, view, iterate, save and publish reports/decks/docs/PDFs/illustrations without meeting git, a shell, or a provider setting. Five pillars: a dev/user **mode** primitive driving system-prompt + tool-group gating at the `prepareSessionConfig` choke point (provider-agnostic; Claude is append-only); **deliverables** (artifacts grown past HTML-only into editable, exportable, multi-format, iterated via a Refine-style diff loop); **non-code Save** (git as an invisible backup engine, implicit `git init`, no remote); a **provider-neutral Connectors layer** (packaged MCP + greenfield OAuth, fanned out to every provider, data in / deliverables out) - **OAuth half SHIPPED**: daemon broker (DCR + PKCE + loopback redirect + silent refresh), catalog rebuilt against cited vendor endpoints after the first pass shipped ~70 placeholder entries that could not start, add-time connect-and-enumerate gate, and connector secrets finally redacted on the wire; see [docs/connectors.md](../docs/connectors.md). Remaining: fan the connector tool surface out past the openai-compat path; and **view-and-verify** (render → show proof → revise). Authoring model decided: structured intermediate + export, not opaque office files. Carves against [agent-orchestration](agent-orchestration/agent-orchestration.md), [graph-templates](graph-templates/graph-templates.md), [claude-extensions](claude-extensions/claude-extensions.md)"
- time: "2026-08-08T06:19:52.551Z"
  kind: "note"
