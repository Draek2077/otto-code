...
- time: "2026-08-08T06:18:02.234Z"
  kind: "decision"
  summary: "Knowledge page created."
- time: "2026-08-08T06:18:02.234Z"
  kind: "evidence"
  summary: "Migrated from `projects/visualizer-pip/visualizer-pip.md` and the legacy `projects/README.md` ledger. Legacy status: Partial. Ledger summary: Audio-while-closed **and the PIP window** both shipped - `panels/visualizer-pip-host.tsx`, the toolbar collapse control, four device-local settings fields (`visualizerPipOpen` / `Size` / `X` / `Y`), documented in [docs/visualizer.md](../docs/visualizer.md) \"PIP mode\". The only remainder is **Arena mode, which is deferred indefinitely** - so the fold-in debt is paid and this folder is drainable whenever Arena's plan is judged worth keeping only in `archive/`"
