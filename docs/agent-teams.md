...
- **A "loading" provider is not an unavailable one.** The daemon's first snapshot for a cwd lists every provider `status: "loading"`, and `checkPersonalityAvailability` only accepts `"ready"` - so "nothing resolves" during that window means _ask again_, not _nothing is available_. `useFormRolePersonality`'s one-shot default applies the team's holder the moment it first resolves, and only gives up once the snapshot is **settled** (no provider still loading).
