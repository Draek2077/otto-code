...
- Use it on components that re-render independently and own visible chrome. It is applied to `Composer` and to the agent panel's composer column.
- Do **not** blanket-wrap the message list. `NamedTheme` calls `UnistylesShadowRegistry.flush()` from a `useLayoutEffect` on every render, so one wrap per message would put a native flush on the streaming hot path. Bubble fills that need the black ramp should take an explicit style override instead.
