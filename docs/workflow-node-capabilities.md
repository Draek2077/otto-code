...
Engine: `packages/server/src/server/workflow/graph-engine.ts`, with one module per
concern beside it. Schemas: `packages/protocol/src/workflow.ts`.
...
`packages/server/src/server/workflow/node-output.ts`
...
`packages/server/src/server/workflow/edge-conditions.ts`
...
`zod-aot` loads `packages/protocol/src/workflow.ts` at codegen time, so a schema
