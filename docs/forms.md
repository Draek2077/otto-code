...
### Settings search targets and nested forms

The Settings shell owns search requests in `screens/settings-search/target.tsx`.
Rows register explicit semantic IDs rather than looking up visible English text.
`SettingsField`, `SettingsSelectField` and the Settings heading adapter supply an
optional `Label` renderer to the existing primitive. A `field={false}` selector
uses its existing selected-value label. These adapters add no row wrapper and
leave theme fonts, sizing, focus rings and platform gates with the control.

Interactive targets use `SettingsButton`, `SettingsPressable`,
`SettingsEditingTextInput` or `SettingsMenuItem`. They forward refs and layout to
the same control; ordinary tab order, input reset/caret ownership, menu selection
and action callbacks remain with that control. A search never invokes Save,
Delete, Connect, tool fetching or another side effect to expose a destination.

Nested editors retain explicit entity choice. `SettingsAdaptiveModalSheet` and
`SettingsTabbedModalSheet` compose the sheet's `contextBridge`; an independently
mounted host carries the optional setting ID explicitly and uses the context-only
`SettingsSearchProvider`. The existing `AdaptiveModalSheet` body and
`TabScrollView` expose their scroll/content refs through `ScrollViewportContext`.
Native search measures against the nearest actual viewport; fixed controls can
receive accessibility focus without an invented scroll container. Unmounted or
not-yet-laid-out targets stay pending, and navigation invalidates old measurements.

Input keyboard coordination is separate: native `EditingTextInput` uses
`SheetAwareTextInput`, which reads `BottomSheetScope` and chooses the existing
`BottomSheetTextInput` inside a sheet. Screens do not maintain a second sheet
input registry. `AdaptiveTextInput` retains explicit `resetKey` replacement through
the editing handle; changing a seed value alone does not overwrite an active edit.

Plugin Settings discovery projects the live, host-scoped plugin registry after
its capability gate. It uses the plugin's registered screen identity and route;
it does not copy registration state into another catalog. Static built-in search
rows remain in `outputs/settings-inventory/settings-index.md`, generated by
`scripts/generate-settings-search-catalog.mjs`.
