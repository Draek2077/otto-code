...
Opening or resuming a chat fetches one bounded latest tail page. Older history remains
...
Provider message IDs are not guaranteed for every displayed item. Otto-generated system errors are one example. Rendered item indices are not durable either because pagination and projection can merge source rows.
...
the Otto timeline, so one visible response can span several canonical turns without a user message
