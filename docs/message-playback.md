...
- the typewriter reveal (`hooks/use-revealed-text.ts`) has finished laying it out.
...
`isTurnTail` down from the authoritative open turn ID; when that turn closes, its
final message counts as finished.
...
Auto-speech deliberately **keeps reading while the app is backgrounded**. A phone
in a pocket or a tab you switched away from still receives completed segments
through its headless source. Playback never waits for a visual reveal timer and
does not depend on a mounted message row.
...
- **It does not wait for the typewriter.** A segment is speakable when the model has
  ...
  all for a chat that is not on screen. Visual presentation and synthesis consume
  that finished text independently.
...
`voice/auto-speech-segments.ts` selects assistant segments by the daemon's turn ID.
The last assistant item remains unfinished while that same turn is open. A later
action in the turn closes the preceding prose; closing the turn releases its final
paragraph. Optimistic prompts and items from other turns do not change that rule.

The producer consumes `Agent.turn`, shared with the transcript's authoritative
lifecycle. It remembers the turn it watched so late final material can still be
spoken, including when the next turn opens in the same store update. A status-only
change cannot reopen that finished reply. A source mounted during reconnect waits
for material belonging to the identified turn before seeding its already-written
segments, so loading that snapshot does not read its existing paragraphs aloud.

Segments are keyed by (group, block index), never by item ID. The live head becomes
`<group>:block:<n>` when it flushes into the canonical tail; that rewrite leaves the
segment's identity unchanged.
