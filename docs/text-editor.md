...
**Results are presented as Changes is**, minus the diff: a file header row (icon, name, muted directory, match count) that sticks while its own results scroll, then the file's matches as a code well - `border`-framed, on the code surface, with a line-number gutter and syntax-highlighted source (`project-search-code-block.tsx`). There is no tree; the file list is flat. Three consequences worth knowing before changing it:

- **A row is a line, not a match.** Several hits on one line collapse into one row with every hit lit up (`buildSearchDisplayLines`, `project-search-code-lines.ts`), because a code view that repeats a line per hit reads as broken. Replace selection therefore covers the line: two hits on one line cannot be checked apart.
- **A hit takes an inline note.** The gutter carries the review surface's comment button on hover, and a saved note renders under the line, exactly as in Changes (`InlineReviewGutterCell` / `InlineReviewThread`). The notes live in a bucket of their own (`buildSearchNoteDraftKey`), **not** a diff draft key: a diff bucket's attachment is built by resolving each comment into a hunk, so a note on a line nobody changed would be written into a bucket that silently drops it on the way to the composer. `buildSearchNoteAttachmentSnapshot` quotes the matched line instead. Both panes replace only their own review pill in the workspace attachment scope, keyed by draft key, so a Changes review and a set of search notes can be attached at once.
- **Preview windows are re-projected.** The daemon centers each match's `lineText` window on its own hit, so matches sharing a row carry different windows. The row renders the first one and maps the others into it by absolute column; a hit outside the window keeps its selection but draws no highlight.
- **The code well is the diff viewer's, minus the diff.** Same rail insets and divider, same marker column (a context line's blank marker, which is what keeps the code column of a result and of a diff on one x), same code metrics, same muting of an unhighlighted line. The add/removed tints and the two-coordinate gutter are the only things left behind. Change one of these and change the other.
- **Highlighting is the shared find language.** Lines are tokenized one at a time (they are disjoint lines lifted out of a file - joining them would let an unterminated string bleed into the next), then hits are cut out of those tokens by `splitTokensForMatches` and tinted with `findHighlightStyles` - the same splitter and the same amber as the read-only preview's find. Do not introduce a second highlight colour here.
...
On desktop, files served by the local host offer a centered **Reveal in Explorer** button beneath the binary-preview message and editor load errors (including non-text files). The label uses the OS file manager's name, and the action reveals the file through the shared desktop open-target bridge. It is hidden for remote hosts and clients without that bridge.
