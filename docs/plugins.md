...
app themes, and composer attachment sources from one `index.ts`. Otto executes the server contribution in a
...
Otto compiles TypeScript and TSX when loading the plugin, so these packages are development dependencies only.
The generated declaration file supplies `@otto-code/plugin` and `@otto-code/plugin/server` types until the
SDK is distributed as a public package. Regenerate new plugins with the matching Otto CLI when the
...
tail from the host plugin settings or with `otto plugin logs <id>`. Reload, disable, and process
...
Shared files import contracts from `@otto-code/plugin/server`. Client files import hooks from
`@otto-code/plugin`. Plugin UI runs on desktop and mobile across multiple themes: color every
...
`useOtto()` and the handler's `{ otto }` context expose the same `OttoApi`: workspaces, agents,
