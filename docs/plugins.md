...
Local plugins contribute agent providers, lifecycle and creation hooks, daemon RPCs, native app surfaces, workspace panels, Command Center items,
...
Otto executes `index.server.ts` in a subprocess and `index.client.tsx` in every connected app.

> **Trust every plugin you add.** `otto plugin add` and `otto plugin install` mean “I trust this codebase.” Plugins are unsandboxed: server code and Git preparation commands run with the daemon user's access on the daemon host, and client contributions run inside Otto. The repository's dependencies and future updates are part of that trust decision. With `--host`, preparation runs on that remote daemon host.

## Compatibility and ownership

Otto's package release and the implemented Paseo plugin API are separate.
`packages/protocol/src/plugin-compatibility.ts` declares API 0.8.0. Both app and
daemon check `requirements.paseo` against that API version; the scaffold pins
`@otto-code/plugin` to the Otto CLI package version independently.
`manifest.ts` accepts `otto-plugin.json` or `paseo-plugin.json` and rejects an
ambiguous pair before preparation or evaluation.

`packages/plugin/src/runtime-specifiers.ts` owns the allowed SDK entries for both
`@otto-code/plugin` and `@getpaseo/plugin`. The compiler, app evaluator and server
process consume this map. The root is shared; `/client` and `/server` own runtime
capabilities. Old pre-split entrypoints and `/client/host` author imports remain
rejected. Upstream `usePaseo`/`paseo` aliases refer to the same Otto API; provider
records cross the explicit upstream-provider boundary adapter, not a second runtime.

The current guide is `public-docs/plugins/v0.8/`. The v0.7 reference is historical
and must not be used to justify restoring the mixed root entry.
...
npm install
npm run typecheck
otto plugin install /absolute/path/to/my-plugin
otto plugin install /absolute/path/to/my-plugin --id another-runtime-id
otto plugin ls
```
...
runtime-safe: run `otto reload` after editing `config.json`. Enabling starts every configured,
...
The directory contains a manifest declaring identity and Paseo API requirements, one optional entry per runtime, runtime-owned
...
The generated `package.json` declares `@otto-code/plugin` and the other host modules as development
dependencies for local typechecking and tests. Otto compiles TypeScript and TSX and supplies the
...
Declare the supported Paseo API range and keep it current when adopting newer APIs. See the
...
backend code can access the daemon machine, while client contributions run inside the Otto app.

Source changes are explicit. Run `otto plugin reload <id>` to stop and fully tear down the old
plugin before compiling and starting from disk. A failed reload stays failed; Otto does not restore
...
A failed plugin remains configured. After correcting its source or migrating its API requirement,
reload the same installation to restore its registrations. The app reevaluates the replacement
bundle; missing panels and plugin settings can become available again. This recovery does not
resume an interrupted provider turn. Provider session restoration requires the persistence
contract described in the provider guide.
...
otto plugin add https://gitlab.com/group/repository.git
otto plugin add https://git.example.com/owner/repository.git
otto plugin add owner/monorepo:plugins/review
otto plugin add owner/repository --ref main
otto plugin ls
otto plugin update review
...
Removing a Git source deletes Otto's managed checkout.
...
shell command strings are rejected. Otto starts the executable directly, without a shell, from the
plugin directory in the staged checkout. It never detects lockfiles or chooses a package manager.

On install and every update, Otto resolves the exact Git revision and manifest, runs the declared
commands, then validates, compiles, and activates the candidate. It logs each argv command and its
output in the daemon log. If a command fails, the error includes its output, Otto discards the
...
Server contributions can write to stdout and stderr with normal Node logging. Otto adds `[otto]`
...
tail from the host plugin settings or with `otto plugin logs <id>`. Git preparation commands are
recorded in `$OTTO_HOME/daemon.log` before a plugin exists, rather than the plugin log tail. Reload, disable, and process
failure retain the tail; removing the plugin clears it. Daemon restarts do not retain the tail, but
structured copies remain in `$OTTO_HOME/daemon.log`. Plugin output can contain secrets, so do not
...
Shared files import contract helpers and types from `@otto-code/plugin`. Server handler files import
`PluginHandlerContext` from `@otto-code/plugin/server`. Client files import Otto UI from
`@otto-code/plugin/client/react-native`. Its `Icon` resolves a Lucide name using the client's installed icon
...
| ----------------------------------------------------- | -------------------------------------------------------------------------- | ---------------------- |
| `@otto-code/plugin`                                   | Shared data, schemas, RPC/settings definitions, runtime-neutral helpers    | Shared code only       |
| `@otto-code/plugin/server`                            | Server contribution/handler contexts and lifecycle contracts               | Shared and server code |
| `@otto-code/plugin/server/provider`, `/server/acp`    | Server provider contracts and adapters                                     | Shared and server code |
| `@otto-code/plugin/client`                            | Client contribution contexts, hooks, navigation, and UI contribution types | Shared and client code |
| `@otto-code/plugin/client/react-native`, `/client/ui` | Host-provided UI components                                                | Shared and client code |
| `@otto-code/plugin/client/host`                       | App-owned rendering integration; not a plugin-author entry                 | Shared and client code |
...
The contribution function must return cleanup. Server cleanup may be async; Otto waits for it when
the plugin is reloaded, disabled, removed, disconnected, or shut down. Cleanup is for resources
created by plugin code. Otto removes registered contributions, unmounts surfaces, clears query
...
Otto owns the route, screen header, Lucide icon validation, close action, theme DTO, layout facts,
...
Otto gives each plugin installation its own query client.

`useOtto()` and the handler's `{ otto }` context expose the same `OttoApi`: projects,
workspaces, agents, terminals, providers, and daemon config. They do not expose connection lifecycle. A surface borrows the
selected host's existing connection; switching the screen's host changes both `useOtto()` and
...
Use plugin RPC for plugin-specific backend behavior that is not a normal Otto operation.
...
When the same plugin contribution exists on multiple hosts, Otto shows it once in the sidebar and
...
Command Center callbacks use the selected host's existing `OttoApi` for normal Otto operations.
...
Provider settings are toggle/select data that Otto renders in the composer. Keep private options in
...
`@otto-code/plugin/server/acp`. Its transformer hooks cover narrow vendor differences; do not translate the
...
Otto memoizes by source-item reference and derives every replacement ID from the source identity, so
...
replaces it, and an empty array removes it. Output `data` must be JSON-compatible. Otto adds the
...
the same `otto`, `rpc`, `openSurface`, workspace, agent, and `openPanel` capabilities as the matching
...
Otto owns the autocomplete row, input clearing, and error toast. It never sends the command text to
...
Register a declarative attachment source backed by a plugin RPC. Otto owns the attachment menu,
...
Screens mount in the existing Host Plugins page. Settings search projects screen descriptors from
the live plugin registry in `screens/settings-search/plugin-projection.ts`; it does not own a
second registration catalog. The route keeps host, installation and screen identity.
...
`addTheme` takes a small light or dark palette and a display name. Otto expands it through the
