...
Otto client UI translations live in `packages/app/src/i18n`.
...
- Batch 4H migrated Sessions screen chrome and AgentList local UI copy: date section headers, local status labels, fallback session titles, badges, load-more/empty states, and the archive action sheet. Chat titles, project paths, provider icons/labels, host labels, relative timestamps, and raw runtime data remain untranslated.
...
- Batch 4O migrated remaining small utility chrome: workspace route gate states/actions, compaction markers, archived-chat callout, web browser fallback, desktop quitting overlay, and image drop overlay. Host names, host status values, browser IDs, token counts, and raw route errors remain runtime values.
...
- Batch 5C paid the pre-0.7.0 debt: the Solution view and its host setting (seeded English in all
  seven non-English locales) were really translated, and the setup wizard, Refine and the
  personality-memory surfaces were migrated out of inline English and translated. It also settled
  the **destructive-dialog resolvers** - `history/delete-dialogs.ts` and the three subagent
  resolvers beside it now read their copy from i18n like every other confirmation (see
  "Dialog resolvers" below), and removed the dead `editor.outOfProject.editOutside*` /
  `editOther*` keys left by the edit-outside change, whose surviving `badge` strings were also
  still translations of a superseded English source.
- Batch 5B drained the whole English-literal backlog: 713 keys that still held their English source string were translated across all seven non-English locales, dropping the fallback ratio from ~29% to ~4% and letting the guard tighten from `0.30` to `0.08`. The same pass repaired a systematic defect in the earlier machine-translated strings - lost whitespace around protected terms, which had produced runs like `ClienteDaemonno disponible`, `etiquete@fileso use/commandsy/skills`, and `queOttoutiliza` - and corrected mistranslated verbs where the machine pass had picked the wrong sense (`Commit` as "commettre/يقترف/Comprometerse" - a crime, not a git commit; `Back` as French "Dos" - the body part; `Archive` as a noun where the UI needs the verb). Git porcelain verbs (`Commit`, `Pull`, `Push`) now stay as the English command in every locale except Japanese, which uses the established katakana forms.
- Korean (`ko`) arrived with the v0.4.0 upstream merge (see [upstream-merges.md](upstream-merges.md)): resource coverage, language selector labels, i18next registration, and system-locale mapping for `ko` and Korean regional locales.

## Dialog resolvers

A **dialog resolver** is a pure function that returns a `ConfirmDialogInput` - title, message,
confirm/cancel labels - for one gesture: `resolveDeleteAgentDialog`, `resolveArchiveSubagentDialog`,
`resolveClearCompletedDialog`, `resolveDetachSubagentDialog`. They are pure so the exact copy can be
asserted in a unit test, which is the right call for text that precedes an irreversible action.

**They are translated, as a family.** Confirmation text is inside the translation scope above, and a
destructive dialog is the worst place in the app to make someone read a second language: consent to
an irreversible action whose wording you could not read is not consent. Being pure does not exempt
them - pure helpers call `i18n.t(...)` directly (Batch 4Y), so the resolvers keep their signatures
and their tests.

Two things stay literal inside them:

- **Provider display names** (`Claude Code`, `Codex`, `GitHub Copilot`, `OpenCode`, `Pi`) - product
  names, same in every locale. The neutral fallback ("The agent provider") _is_ translated.
- **Anything quoted back from data** - a chat title, a subagent name, a path.

Their tests assert English because `en` is the default language; add a `changeLanguage` case (as
`delete-dialogs.test.ts` and `git/policy.test.ts` do) when a resolver's translation is worth
guarding.

## What stays in English

A string that equals its English source is not automatically a gap. These are deliberate and must not be "fixed":

- Brand and product names: `Otto`, `GitHub`, `Bitbucket Cloud`, `OpenAI`, `Claude Code`, `.NET`, `Node`, `Python`, `Go`, `Rust`, `Java`.
- Quote attributions on the Open project screen - real people and works, left in their original script.
- Code and command placeholders: `sk-…`, `claude`, `my-project`, `main`, `--dangerously-skip-permissions`, `https://github.com/owner/name.git`.
- Pure-interpolation strings such as `{{completed}}/{{total}}` and `{{provider}} · {{model}}`.
- Terms a locale genuinely shares with English - Spanish `Error`, French `Mode`, Portuguese `Status`,
  French `Agents`, and `OK` in French, Japanese and Portuguese.
- **Model input, not UI copy.** The setup wizard's team blueprints in
  `packages/app/src/screens/setup-wizard/presets/` carry team prompts, functional cores and persona
  names that go to the model; they stay English. Only the two strings a user reads - the blueprint's
  name and tagline - live in i18n, and the name is deliberately one key for both the card and the
  team it installs, so the two can never disagree. Refine's preset `instruction` is the same case:
  the chip label and description are translated, the seeded instruction is not.

Product nouns that the app keeps in English as UI labels (`Workspace`, `Terminal`, `Host`) stay English inside translated sentences too, except in Japanese, which uses ワークスペース and ターミナル. Keep the surrounding grammar native: `Configuración del Workspace`, not `ConfiguraciónWorkspace`.
