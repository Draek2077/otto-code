...
Configured `otto.json` scripts can be managed without addressing their backing terminal directly:

```bash
otto script ls [--cwd <path> | --workspace <workspace-id>]
otto script start <name> [--cwd <path> | --workspace <workspace-id>]
otto script stop <name> [--cwd <path> | --workspace <workspace-id>]
```
...
Otto sets these when it forwards a request to a workspace service:
