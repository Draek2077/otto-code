...
- **Out-of-band commands are busy work too.** `/compact` is out-of-band on Codex, Pi and OMP (`tryHandleOutOfBand`), so it allocates no foreground turn - and for a while that meant the queue had no drain edge on those providers at all: a prompt queued while the context was being rewritten sat in the Queue track forever, because nothing ever finished a turn to release it. `tryRunOutOfBand` now reports the agent `running` for the lifetime of the handler and `finalizeOutOfBandRun` settles it with the same drain decision `finalizeForegroundTurn` makes. Three things follow from the busy window, and all three are the point: `delivery: "queue"` parks the next prompt instead of dispatching it into a session mid-rewrite; the composer offers **Queue** rather than **Send** (`isCompactionActive` needs a live turn to be sure); and an _interrupting_ send is downgraded to a queue (`isBusyOnlyWithOutOfBandRun`), because there is no turn to cancel and cutting in would run the next turn against a half-summarized conversation. The completion transition is silent - a side-effect command finishing is not an assistant reply, so it raises no unread badge.
  - A provider's handler must therefore stay open for the **whole** operation. Codex's `thread/compact/start` answers as soon as the request is accepted, so the adapter waits on the `thread/compacted` notification (or the completed `contextCompaction` item) before returning, under a five-minute cap so a build that never reports cannot pin an agent busy.
...
Provider-descriptor child sessions expose **Stop** while running and **Archive** once terminal in both subagent presentations. Read-only means the user cannot prompt the child; it does not remove lifecycle authority. Stop targets the provider child where the adapter supports it (Codex, Claude, OpenCode). Otherwise Otto requires explicit confirmation before closing the owning agent session, which stops the parent session and its other work. Archive records a tombstone on the parent chat in host storage and broadcasts it to connected clients; replay cannot restore an archived row. A failed stop is reported and does not proceed to archive. The client requires the `providerSubagentControl` host capability for these operations.
...
An observed row may shadow a provider descriptor for the same run. Archiving the observed row also persists the descriptor's parent-owned archive tombstone, matched by the observed key against the descriptor id or tool-call id. A directory refresh omits archived observed rows; without the descriptor tombstone it would reveal the duplicate and make Clear appear to require a second click. If tombstone persistence fails, the observed row stays visible for retry.

**A native subagent going `idle` is not terminal.** `isSubagentRowTidyEligible` counts `idle` as terminal only when `attend === "observed"`. A Claude Task completion maps to idle, but a native `create_chat` subagent idles _between turns_ and must never auto-tidy out of the active list.
...
| `labels["otto.parent-agent-id"]` | `string?`     | Parent agent ID, set automatically by `create_chat` when `relationship.kind === "subagent"` |
