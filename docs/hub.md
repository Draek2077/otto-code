...
`otto hub connect [url]` from the daemon machine with an explicit API key or matching stored CLI login.

The human CLI login and daemon relationship are separate identities. `otto hub login [url]` stores a durable organization-scoped CLI credential keyed by normalized Hub origin under `OTTO_HOME`. Origin resolution uses explicit command input, `OTTO_HUB_URL`, active login, then `https://hub.otto-code.me`. Connect uses exact-origin authority to request a one-time enrollment token, then passes only that token to the daemon. The daemon generates and persists its own relationship credential.

## Connection and authority

The daemon enrolls over HTTP(S), then opens and maintains a direct outbound WebSocket to the Hub.
The Hub never discovers or acquires the daemon through Otto's relay. The relay remains an optional
encrypted path for normal Otto clients and has no role in Hub enrollment, authentication, dispatch,
or reconnects.
...
daemon, execution, and agent identity with that terminal state. Otto never stores or automatically
replays the original prompt. A duplicate create returns the existing agent without starting another
turn.

Every Hub execution creates a fresh Otto workspace. The workspace owns the execution's agents and
...
that workspace's agents and terminals, then removes Otto-owned backing directories only after their
...
`otto hub disconnect` disables socket reconnect and execution authority before making one bounded
...
`otto hub logout` removes only the active human CLI credential and preserves credentials for other origins. Interactive logout inspects and optionally disconnects a same-origin daemon before deleting the login; a failed requested disconnect preserves the login. JSON and noninteractive logout never prompt or disconnect implicitly.

## Cross-repository compatibility

The consumer implementation lives in Otto Cloud. Cloud owns its copy of the Hub wire schemas and
has no Otto runtime or build dependency. Cross-repository end-to-end verification separately builds
a Otto source checkout and exercises the real daemon, CLI, direct WebSocket, Cloud service, and
