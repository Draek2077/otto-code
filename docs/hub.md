...
When that agent belongs to an active Otto-owned worktree workspace, the daemon also archives the
...
`otto hub disconnect` disables socket reconnect before requesting remote revocation. If the Hub is
