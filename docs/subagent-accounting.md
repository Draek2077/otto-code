...
7. **Does its input figure already include the cached slice?** `AgentUsage`'s three input leaves are **disjoint** and are summed downstream, which is Anthropic's shape. OpenAI's is the opposite: `input_tokens` is the whole prompt and `cached_input_tokens` is a slice **of** it. Subtract at the adapter, as `parseStreamUsage` (openai-compat) and `toAgentUsage` (Codex) both do. Getting this wrong is invisible in review and doubles every token figure the provider produces once prompt caching warms up, because a warm prefix makes the cached slice nearly the whole prompt.
8. **Is `usage` reported once per turn, or once per model request?** Codex reports per request, and one turn is many requests. Publishing only the newest one books a single round and discards the rest, so accumulate across the turn (`accumulateCodexTurnUsage`) and reset at both turn boundaries. Spend leaves add; `contextWindow*` leaves are occupancy and take the newest reading instead (see [Spend is not occupancy](#spend-is-not-occupancy)). A turn that dies before its first request must report nothing rather than re-book the previous turn.
9. **Does a failed or interrupted turn still report what it burned?** It burned every request it made, and the manager books `usage` on `turn_failed` and `turn_canceled` as well as `turn_completed`. Omitting it there makes Esc-heavy and retrying sessions look free.
