...
processes and subscriptions while retaining its Otto identity, persistence handle, timeline,
...
same Otto agent ID. Provider history is not appended again when the canonical timeline is already
primed.
...
Claude reports this background process exit. Other providers must report runtime death through their
adapter event contract; registration alone does not prove equivalent exit coverage.
...
Agents launch chats through Otto's `create_chat` tool. Agent-scoped creation is asynchronous. A bound subagent stamps `otto.parent-agent-id`; an explicit detached relationship keeps the caller context for launch policy without binding archive lifecycle. Placement and relationship are separate choices. The shared creation path preserves provider, profile/personality, tool access and unattended policy across cloud and local providers.
...
The parent-archive policy requires detaching a subagent instead of archiving it when either condition holds:
...
every managed subagent present in its tabs with `otto.open-agent-tab.<client-id>=true` through the
...
Some providers can create their own child sessions inside one provider runtime. OMP's task tool reports these with `child_session` events; `AgentManager` imports the live provider handle, stamps `otto.parent-agent-id`, and surfaces the result as a normal subagent in the parent's subagents track.

The provider still owns the underlying runtime. Otto keeps an agent record so the child can be opened, tracked, archived, and cascaded with the parent, but prompts and history hydration route through the provider adapter for that native child handle.
...
1. Serialize the operation with other lifecycle mutations for that agent.
2. Snapshot the session, persist `archivedAt`, normalize status and notify subscribers.
3. Close the runtime and invoke the provider's native archive hook.
4. Resolve child lifecycle according to the parent-archive policy above.

Reload and registration failures preserve the durable closed record and its context. Failed cleanup
notifications do not replace the original lifecycle error, and runtime closure is deduplicated. A
failed interrupt cannot authorize a replacement runtime. Provider lifecycle hooks and creation
transforms belong at their explicit manager/runtime boundaries, not in individual UI callers.
...
leave both Otto's `archivedAt` and the provider's native archive state unchanged. **Unarchive** remains
...
provider session can be archived outside Otto while its Otto agent remains active. Interactive
...
- **Otto subagents** include managed chats and retained observed projections. The selector in
  `packages/app/src/subagents/select.ts` includes unarchived direct children and descendants reached
  through observed intermediates; an attended intermediate owns its own track. Otto rows preserve
  personality, usage, tool count and current-tool readouts when reported.
...
Clicking either kind opens a workspace tab. A Otto subagent tab is a normal interactive agent pane. A provider subagent tab is a read-only timeline pane with no composer, detach, rewind, or fork actions. Otto routes supported Stop and Archive actions through the provider-subagent control contract; a parent-scoped stop requires explicit confirmation and must never silently interrupt the parent or siblings. It shows its own direct children in a subagents track. Both panes use `AgentStreamView`, so message, reasoning, tool-call, and layout rendering stay identical.

Provider timelines use the same structural timeline item format but deliberately have a separate lifecycle and transport. A provider thread/session identifier is not a Otto agent identifier, and closing its tab is always layout-only.
...
- **Effort is only reachable through hooks.** It appears nowhere on the message stream at any depth, and the level Otto requests is not necessarily the level that runs — a model that does not support it is silently downgraded. A hook firing inside a subagent reports the active post-downgrade level next to its `agent_id`, which is the same id `task_started` calls `task_id`.
...
Archived Otto subagents disappear from the track, by design. To remove one from the track without closing its tab, use the **archive button** on the row — it opens a confirm dialog and archives the subagent on confirm. Provider-owned row controls use `use-provider-subagent-control.ts`, keeping provider identity separate from managed agent IDs. Unsupported operations report their capability limit; they do not become whole-parent actions silently.

The **Archive finished** row at the foot of the panel covers every finished row. It archives eligible idle or errored managed Otto subagents one at a time. Provider-owned outcomes follow their provider control path; failures remain visible and retryable. Running and initializing children remain in the track. If a hidden provider child starts running again, the app brings it back to the track.
...
A parent that spawns many subagents will see the panel's list grow; the pill only counts them. Managed Otto subagents can be archived individually or with **Archive finished**. Provider-owned rows use the provider control path; the app reports individual failures so the user can retry without losing those rows.
...
| `labels["otto.parent-agent-id"]`            | `string?`     | Parent agent ID, set automatically for agent-scoped creation and removed by detach |
| `labels["otto.open-agent-tab.<client-id>"]` | `string?`     | `"true"` protects an open tab on that client; detach clears every matching label   |
