...
Server-side stores live under `$OTTO_HOME` (defaults to `~/.otto`), with one exception: artifacts are stored inside the project directory at `{projectCwd}/.otto/artifacts/` (see [Artifacts](#6-artifacts)).
