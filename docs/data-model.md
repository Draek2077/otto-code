...
`OTTO_GIT_MAX_PROCESS_CONCURRENCY` wins when it and the legacy alias are both set. Restart the
daemon after changing the file or environment. Run `otto daemon restart` for a standalone daemon.
For a desktop-managed daemon, fully quit and reopen Otto Desktop.
