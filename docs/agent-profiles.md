...
**Override semantics.** Selecting a profile in a picker fills the underlying provider/model/effort/mode fields; the user may hand-edit any of them (an explicit per-field override) and **the agent keeps the profile identity** (name, spinner colors, prompt) with the overridden brain layered on. Only an explicit "clear profile" detaches back to a plain provider/model selection. **The provider is the one field that cannot deviate**: a profile on Claude does not describe a Codex agent. Spawn (`Session.applyProfileIdentityToConfig`, `session.ts`) soft-skips the identity when the requested provider differs from the profile's, and the form pickers (`useReapplyOnProviderDrift`, `role-model-personality.ts`) re-apply the selected profile or team slot whenever the form re-resolves onto another provider (hide/show, host change), so the trigger and the spawn never disagree. In tooling, the template applies verbatim and a caller may override individual fields **only when explicitly requested** - no heuristic substitution.
...
**Strict resolution.** The session shell (`Session.resolvePersonalitySnapshotForAgent`, `session.ts`) re-resolves the roster id against the agent's cwd before applying. Unlike spawn's soft-skip (`applyProfileIdentityToConfig` logs and spawns without identity), the live switch **rejects the RPC** when the profile is unknown or out of commission, surfacing the unavailability reason. It warms only the profile's own provider snapshot so a cold workspace doesn't fan out network probes to every provider.
