...
| [docs/agent-personalities.md](docs/agent-personalities.md)                       | Agent Personalities — data model, 7 roles, availability, spawn-snapshot lifecycle, spinner/voice identity, MCP tooling, starter team                                                   |
...
| [docs/git-providers.md](docs/git-providers.md)                                   | Git hosting providers — provider-neutral forge layer (GitHub + Bitbucket Cloud), remote-based resolution, capabilities gating, credential/merge security                               |
...
| [docs/desktop-linux.md](docs/desktop-linux.md)                                   | Linux desktop: packaging (deb/rpm/AppImage), per-format Chromium sandbox, Ubuntu 24.04 AppArmor profile, GPU software-render fallback, crash dialog, diagnostics                       |
...
| ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [projects/agent-teams/agent-teams.md](projects/agent-teams/agent-teams.md)                                                   | Charter: Agent Teams — named per-host groupings of personalities acting as switchable operating templates; one active team at a time (host-scoped `activeAgentTeamId`, instant main-UI switcher), team prompt stacks ahead of the personality prompt at spawn, pickers/Writer routing/`list_personalities` scope to members, role-pill union on team cards, color avatars now → themed image set later                                                                                                                                                                                   |
| [projects/first-time-wizard/first-time-wizard.md](projects/first-time-wizard/first-time-wizard.md)                           | Charter: First-time wizard — full-screen guided setup on first host connection (providers detected → interface mode → personality presets Friendly/Professional/Agentic/Mixed, 12 themed personalities → team templates with role-fill → home); least-setup principle, pristine-roster replace only, device-local completion flag. Absorbs the interface-modes charter ([interface-modes.md](projects/first-time-wizard/interface-modes.md): User vs Developer lens, `useIsDeveloperMode()` gate, surface inventory) as wizard step 2 + a gating phase; depends on agent-teams steps 1–2 |
| [projects/computer-use/computer-use.md](projects/computer-use/computer-use.md)                                               | Charter: Computer use — agents see and control the user's real desktop (screenshot → click/type loop) via a shared `@otto-code/computer-control` library with daemon/frontend executors ([computer-control-library.md](projects/computer-use/computer-control-library.md): scaling math, Set-of-Marks, Wayland strategy, OpenDesk prior-art), layered safety (arming, touch-your-mouse auto-pause), openai-compat vision delivery as Phase 0                                                                                                                                             |
| [projects/dictation-refine/dictation-refine.md](projects/dictation-refine/dictation-refine.md)                               | Charter: Dictation Refine — AI cleanup over dictated text via a latency-ordered ladder (Layer 0 instant deterministic lexical/coding-vocab pass → Layer 1 fast local ONNX punctuation+truecasing model → Layer 2 optional LLM smart-refine reusing the structured-generation provider chain), with a non-blocking "settle" (fast text lands instantly, slower layers replace in place). Adopt OpenWhispr's LLM-post-processing idea, don't bundle the app; provider-neutral, local-LM-Studio proof                                                                                       |
...
| [projects/subagents-cleanup/subagents-cleanup.md](projects/subagents-cleanup/subagents-cleanup.md)                           | Charter: subagents robustness + cleanup pass (post-first-tester) over the existing track/tab/pane, both observed and native — kill the click-to-open 404 (observed rows are ephemeral, `fetchAgent` 404s), status-aware row actions (Stop when running / Archive when terminal), lock tab-close as layout-only, honest provider token cost per row + parent total, freeze runaway names (stop `task_progress.summary` mutating the title), auto-tidy completed rows into a collapsed group                                                                                               |
| [projects/subagent-liveness/subagent-liveness.md](projects/subagent-liveness/subagent-liveness.md)                           | Charter: Subagent liveness signals (subagents-cleanup Phase 6) — close the at-a-glance gap vs. Claude's background-task panel by adding per-row elapsed time (6a, shipped client-only), current/last tool in the subtitle slot (6b, daemon — tool name already resolved but discarded at `agent.ts` `appendObservedSubagentTaskEvent`), and a monotonic tool-use count (6c, daemon — mirror the Phase 4 `cumulativeTokens` accumulator); all additive optional protocol leaves, observed-Claude proof then native then remaining providers                                               |
| [projects/steer-queue/steer-queue.md](projects/steer-queue/steer-queue.md)                                                   | Charter: Queued steering messages — a non-interrupting `delivery: "queue"` mode on `send_agent_prompt` alongside today's interrupt (`replaceAgentRun`); buffers a prompt as the next turn and drains it in `finalizeForegroundTurn` when the turn finalizes to idle (context preserved, process alive). Lives in the turn lifecycle above the provider, so it ships for all providers at once; additive `queuedMessageCount?` snapshot leaf + row badge/clear; open question whether @mentions/schedule fires should queue-by-default                                                    |
| [projects/safe-unattended/safe-unattended.md](projects/safe-unattended/safe-unattended.md)                                   | Charter: Safe unattended runs — schedules/artifacts stop using bypass; Claude-Desktop-style posture (`dontAsk` deny-if-not-pre-approved target, Auto classifier where the model supports it, daemon deny-responder for escalations), internal + surface-only-on-problem; Phase 0 (per-model Auto eligibility + hiding) shipped                                                                                                                                                                                                                                                           |
...
| [projects/mobile-daemon/mobile-daemon.md](projects/mobile-daemon/mobile-daemon.md)                                           | Charter: Mobile daemon — an embedded, API-native daemon running on the phone itself ("This device" host); `daemon-core` extraction with six platform seams, in-memory client transport, Anthropic Messages API + openai-compat adapters, mobile tool flavor; agent CLIs can't run on phones, so no CLI providers                                                                                                                                                                                                                                                                         |
...
| [projects/site-demos/site-demos.md](projects/site-demos/site-demos.md)                                                       | Charter: marketing-site demo assets — staged fake repos with real code + git history, isolated seeded daemon (e2e harness), Playwright scenario scripts with real provider runs, PNG + MP4/WebM capture into `packages/website/public/demos/` with a step manifest                                                                                                                                                                                                                                                                                                                       |
| [projects/agent-personalities/agent-personalities.md](projects/agent-personalities/agent-personalities.md)                   | Shipped 0.5.0 — durable architecture folded into [docs/agent-personalities.md](docs/agent-personalities.md); charter retains the build sequence and the one open product decision (persisting a client schedule-form personality binding)                                                                                                                                                                                                                                                                                                                                                |
| [projects/git-hosting-providers/git-hosting-providers.md](projects/git-hosting-providers/git-hosting-providers.md)           | Shipped 0.5.0 — durable architecture folded into [docs/git-providers.md](docs/git-providers.md); charter retains locked product decisions and the deferred set (Bitbucket Server/Data Center, GitLab, Gitea, GitHub Enterprise custom hosts, OAuth)                                                                                                                                                                                                                                                                                                                                      |
| [projects/vertical-tabs/vertical-tabs.md](projects/vertical-tabs/vertical-tabs.md)                                           | Charter: optional per-pane vertical tab rail alongside the existing horizontal tab strip — mirrors the `SplitGroup.direction` precedent as `SplitPane.tabOrientation`, default set in Appearance settings, mixed orientations across panes in a split layout                                                                                                                                                                                                                                                                                                                             |
