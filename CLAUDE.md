...
| [projects/agent-personalities/agent-personalities.md](projects/agent-personalities/agent-personalities.md)                   | Charter: named per-host agent templates (provider→model, canonical effort, mode, personality prompt, 7 roles, spinner colors) selectable atop every model picker and spawnable by ID via Otto tooling; unavailable when bindings don't resolve, Orchestrator-only enumerates others |
