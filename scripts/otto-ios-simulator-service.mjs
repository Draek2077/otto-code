...
const appProductName = "OttoDebug";
const appScheme = "otto";
const preferredSimulatorType = process.env.OTTO_IOS_DEVICE_TYPE || "iPhone 16 Pro";
const ottoPort = requiredEnv("OTTO_PORT");
...
async function main() {
  ...
  metro = startMetro();
  await waitForUrl(`http://127.0.0.1:${ottoPort}/.sim`);
  console.log(`iOS preview: ${process.env.OTTO_URL || `http://127.0.0.1:${ottoPort}`}/.sim`);
...
function launchApp() {
  const metroUrl = encodeURIComponent(`http://127.0.0.1:${ottoPort}`);
  run(
    "xcrun",
...
function startMetro() {
  const child = spawn("npx", ["expo", "start", "--port", ottoPort, "--localhost"], {
    cwd: appDir,
    env: {
      ...env,
      OTTO_SERVE_SIM_PREVIEW: "1",
      OTTO_SERVE_SIM_DEVICE_UDID: simulatorUdid,
      BROWSER: "none",
    },
    stdio: "inherit",
  });
...
function requiredEnv(name) {
  ...
  if (!value) throw new Error(`${name} is required; run this as a Otto service.`);
  return value;
}
