...
description: Connect to a Otto daemon, run one coding agent, and read its reply.
nav: Quickstart
order: 51
...
`cwd` is the directory the agent works in. Otto creates the workspace behind it. [Workspaces](/docs/sdk/workspaces) covers reusing one instead.
...
| `permission` | The agent needs a person to answer a permission request in Otto. |
...
The agent keeps running on the daemon and stays visible in Otto. A later program reaches it again by ID:
