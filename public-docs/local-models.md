...
- **Chat turns stream over the standard `chat/completions` endpoint.** Reasoning models that emit a separate reasoning stream have it rendered as thinking output, the same as any other provider's extended thinking.
...
- **Permission modes work the same as any other provider.** Always Ask prompts before edits and commands, Accept Edits auto-approves file changes but still asks before running commands, Read Only offers no write or Otto tools at all, and unattended mode skips confirmation entirely.
