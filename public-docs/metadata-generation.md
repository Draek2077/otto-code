...
description: How Otto uses providers to generate branch names, commit messages, and pull request text, and how to configure them.
...
Otto asks a language model to write short pieces of text for you so you don't have to. This is separate from the agent you're talking to: it's a small, one-shot call made in the background.
...
Open **Settings → Host → Metadata** and select **Automatic** or **Manual**. In Manual mode, Otto tries the model you choose first, then falls back to the remaining configured models and built-in candidates. Choosing a model replaces the first entry in `agents.metadataGeneration.providers` and preserves the rest of the list.

## Configure a custom fallback order

To configure more than one preferred model or control the exact order, set `agents.metadataGeneration.providers` in `~/.otto/config.json`. Your entries are tried before the built-in defaults.
...
You can steer the wording of each kind of metadata per repository with a `otto.json` file at your repo root. Otto reads it from the committed version of the base branch, the same way it reads worktree config.
