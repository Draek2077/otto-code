...
description: Build, install, share, and update a trusted Otto plugin.
nav: Paseo v0.8 (current)
order: 46
category: Plugins
---
...
> **For the Paseo v0.8 API integrated into Otto.** Use the [v0.7 docs](/docs/plugins/v0.7)
> only when maintaining the historical mixed-entry API.

> **Experimental:** The plugin API is still evolving, so expect breaking changes and updates to
> your plugins as Otto evolves. See the [plugin roadmap](https://github.com/getpaseo/paseo/labels/plugins)
> for planned contribution surfaces.

A plugin is a TypeScript project installed into one Otto daemon. It can add
...
contributions run on every Otto client connected to that daemon, including mobile.

This guide scaffolds a plugin, runs it, and adds a workspace panel to it.

The manifest uses `requirements.paseo` for the integrated API range, currently
`0.8.0`; it does not use Otto's package release number. Otto scaffolds
`otto-plugin.json` and `@otto-code/plugin` imports. An upstream `paseo-plugin.json`
and supported `@getpaseo/plugin` imports may be kept; never place both manifest
filenames in one directory. See [requirements and aliases](/docs/plugins/v0.8/reference#requirements).
...
```bash
otto plugin init /absolute/path/to/workspace-plugin
...
development dependencies for typechecking and tests only; Otto supplies the plugin SDK, React,
...
```text
workspace-plugin/
  otto-plugin.json      # plugin ID and supported Paseo API range
  index.client.tsx       # runs in the Otto app
...
user's access on the daemon machine, and client code runs inside the Otto app. Installing a plugin
...
daemon's `config.json`; after editing the file, apply it with `otto reload --json`. An automated
...
```bash
npm run typecheck
otto plugin install /absolute/path/to/workspace-plugin
otto plugin ls
```

`otto plugin ls` should report the plugin as `running`. Open Otto, choose **Greeting** in the
...
the client is viewing the host you installed into. `otto plugin logs workspace-plugin` shows the
...
```tsx
import { type PluginWorkspacePanelProps, useWorkspace } from "@otto-code/plugin/client";
...
`theme.colors`, and `layout.compact` drives spacing, so the panel works in every Otto theme and on
...
```bash
npm run typecheck
otto plugin reload workspace-plugin
```

A reload stops the old plugin, runs its cleanup, compiles the current source, and starts it again.
A failed reload stays failed and reports its error in `otto plugin ls`; fix the source and reload
again.
...
otto plugin add owner/monorepo:plugins/workspace
...
```bash
otto plugin ls
otto plugin update workspace-plugin
otto plugin update --all
```

`ls` reports runtime state, source details, and the installed commit without contacting the remote.

Otto compiles TypeScript itself, so most plugins need no build step. A repository that must
install a dependency Otto does not provide, or generate files, declares
...
```bash
otto plugin logs workspace-plugin
otto plugin logs workspace-plugin --json
```

The tail includes `[otto]` loading, ready, stopping, and stopped entries, plus compilation and load
...
- [TypeScript SDK](/docs/sdk): the workspace, agent, provider, and config API available as `otto`
