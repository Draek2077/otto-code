...
description: Add a coding agent to Otto directly or adapt an ACP agent.
...
> **For the Paseo v0.8 API integrated into Otto.** Start with the
> [plugin quickstart](/docs/plugins/v0.8) if you have not built a Otto plugin before.

A provider plugin connects a coding agent to Otto without adding it to Otto core. Publish the
plugin in a Git repository and users can install and update it with `otto plugin add` and
`otto plugin update`.
...
- [`provider-direct`](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/provider-direct): sessions, settings, prompts, persistence, child sessions, and a provider-owned timeline renderer;
- [`provider-acp-transformer`](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/provider-acp-transformer): an ACP command template with a Zod-validated vendor edit transformer. Replace `example-acp --stdio` with an installed ACP agent before loading it;
- [`inline-thinking`](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/inline-thinking): a renderer-only plugin that does not implement a provider.
...
Otto requests the catalog before creating a session. Return the choices needed by the agent form:
...
the user timeline item so Otto replaces its optimistic message. Every started turn needs one
...
Otto renders these controls in the composer. A user change arrives as `session.configure`; apply it
...
for provider-specific configuration that Otto does not render. A provider-specific token budget
...
Return an opaque persistence value in `session.opened` or `session.persistence`; Otto stores it
...
Otto refreshes an agent by closing its current provider session and opening it again with current
...
Use `restoration: "core"` when Otto can reopen a session from persistence. A provider-owned child
...
Advertise only capabilities you implement. If steering is unsupported, omit `prompt.steer`; Otto
...
a running tool, or a todo list. Otto derives the live delta and keeps the normal timeline.
...
```ts
import type { AcpTransformer } from "@otto-code/plugin/server/acp";
import { z } from "zod";
...
otto plugin update my-provider-plugin
```

Keep vendor compatibility and releases in that repository. Otto core should only change when the
