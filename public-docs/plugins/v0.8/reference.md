...
> **For the Paseo v0.8 API integrated into Otto.** Return to the [v0.8 quickstart](/docs/plugins/v0.8).

Migrating an existing plugin? Follow the standalone [runtime-entry migration guide](/docs/plugins/v0.8/migration).

Local plugins are directory sources installed into one Otto daemon. A plugin can contribute:

- React Native surfaces and sidebar items to Otto clients;
...
- normal Otto operations through the TypeScript SDK;
- searchable external resources in the message composer.

Plugin code is trusted and unsandboxed. Client surfaces run in the Otto app. Backend contributions run in a subprocess with access to the daemon machine, including its files, processes, credentials, and network.

## Project files

`otto plugin init /absolute/path/to/my-plugin` creates:
...
The required root manifest is `otto-plugin.json`. It contains the default plugin ID and supported Paseo API range:
...
The root manifest is `otto-plugin.json`. An upstream plugin may instead keep
`paseo-plugin.json`; the loader accepts either filename and rejects a directory
containing both. Keep one manifest with the same ID and build commands.

`requirements` is optional. Its supported key is `paseo`, an npm semver range for
the integrated **Paseo plugin API**, not the Otto package release. The current
app and daemon expose API version `0.8.0`. An omitted `requirements.paseo` means
`<0.8.0`, so the current runtime rejects it with migration guidance. Empty strings,
invalid ranges and unknown manifest requirement keys are rejected.

| Range            | API versions accepted by the range                                     |
| ---------------- | ---------------------------------------------------------------------- |
| `>=0.8.0`        | 0.8.0 and later API versions, including future breaking versions       |
| `^0.8.0`         | 0.8.x API versions                                                     |
| `>=0.8.3 <0.9.0` | 0.8.3 through the last 0.8 patch; does not match the current 0.8.0 API |

Prereleases also match ranges satisfied by their stable core: `0.8.0-beta.1`
satisfies `>=0.8.0` but not `<0.8.0`.

`otto plugin init` writes `>=0.8.0` from the integrated API version and pins
`@otto-code/plugin` to the CLI's Otto package version for local typechecking.
Those versions are independent. Raise the API floor only when using a newer API;
an upper bound can exclude a future incompatible API release.

The daemon checks compatibility before install, Git preparation or loading, and
again on startup, enable and reload. An incompatible Git update retains the
installed revision. Each app checks its own integrated API before evaluating
client code; a compatible daemon does not make an older app compatible. A plugin
without a client entry needs no client evaluation.

Compatibility diagnostics retain the upstream API name, for example:
`Plugin "review" requires Paseo >=0.8.0. Your daemon is 0.7.2.` The displayed number
is the runtime's Paseo API version, not its Otto package version.

### Runtime entries

| Entry              | Runtime              | Receives              | Required                                                                        |
| ------------------ | -------------------- | --------------------- | ------------------------------------------------------------------------------- |
| `index.client.tsx` | Otto app, per client | `PluginClientContext` | When the plugin has any UI, callback, theme, or attachment source               |
...
The generated `package.json` installs `@otto-code/plugin` and the other host modules as development
dependencies for local typechecking and tests. Otto supplies their runtime instances. Consumers do
...
Otto builds each bundle from its matching entry. An import from `client/` into the daemon bundle,
...
The SDK root (`@otto-code/plugin`) contains shared data, schemas, and runtime-neutral helpers only.
...
Otto provides these modules to client code:

| Module                                  | Use it for                                                                                        |
| --------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `@otto-code/plugin`                     | Shared data, `defineRpc`, `defineSettings`, `defineAttachmentSource`, `RpcInput`, and `RpcOutput` |
| `@otto-code/plugin/client/ui`           | Named, composable settings components                                                             |
| `@otto-code/plugin/client/react-native` | Otto UI components and UI hooks                                                                   |
| `@otto-code/plugin/client`              | Client contribution contexts, `useOtto`, `useRpc`, `useSettings`, and data hooks                  |
...
Do not import `lucide-react-native`, `react-native-svg`, or DOM libraries. Set contribution `icon` fields to a [Lucide icon name](https://lucide.dev/icons/); Otto validates the name and renders the icon.

### Upstream import compatibility

Otto-authored plugins use the `@otto-code/plugin` entries above. The compiler and
host runtime also recognize the corresponding `@getpaseo/plugin` entries:
shared root, `/client`, `/client/ui`, `/client/react-native`, `/server`,
`/server/provider` and `/server/acp`. Both namespaces follow the same runtime
boundaries and resolve through Otto's host-owned modules. Upstream `usePaseo`
and callback `paseo` aliases address the same host API as `useOtto` and `otto`.
The provider boundary adapts upstream provider records explicitly.

Aliases do not restore obsolete root hooks, old `/ui` or `/provider` entrypoints,
or the private `/client/host` entry. Use the migration guide for those changes.
Keep dependencies aligned with the author namespace you typecheck; do not bundle
a second React, React Native or SDK runtime into the plugin.
...
Otto provides `@otto-code/plugin`, `@otto-code/plugin/server`,
`@otto-code/plugin/server/provider`, `@otto-code/plugin/server/acp`, and `zod` to server code. Backend
...
`@otto-code/plugin/server/provider`. Its connection accepts inputs with `send()` and emits complete state
...
Provider settings are toggle/select descriptors that Otto renders in the composer. Keep
provider-private JSON under `providerOptions`. Host tools arrive as MCP servers in the complete
session config.

Otto refreshes an agent by closing its current provider session and opening it with current
configuration and persistence. Providers re-read external state during `session.open`.

Use `runAcpProvider()` from `@otto-code/plugin/server/acp` to adapt a command-backed ACP. Add transformer
...
allowed. Otto reads and sanitizes the file when the plugin starts; the string is never an inline
SVG or URL.
...
which return `{ update, remove }` handles. The entry cleanup runs before Otto removes remaining registrations.
...
Cleanup can be async. Release timers, watchers, sockets, and other resources created by the plugin. Otto also removes registrations, unmounts surfaces, rejects pending RPCs, closes the plugin's daemon session, and stops its subprocess on reload, disable, removal, disconnect, or daemon shutdown.
...
| ------------------------------- | --------------------------------- | ------------------------------------------------------------ |
| `server.on(name, callback)`     | `(event, { otto, signal })`       | `void` or `Promise<void>`                                    |
| `server.before(name, callback)` | `({ request }, { otto, signal })` | Modified request, or `undefined` to keep it; async supported |
...
Copy [server/inspect.ts](https://github.com/getpaseo/paseo/blob/v0.8.0/plugin-examples/lifecycle-actions/server/inspect.ts)
into your plugin. The helper imports types from `@otto-code/protocol/agent-types`; add
`@otto-code/protocol` at the same version as your plugin SDK to your development dependencies
...
export default function contribute(server: PluginServerContext) {
  server.on("agent.turn_ended", async (event, context) => {
    ...
    if (/out of credits/i.test(text)) {
      await context.otto.agents.ref(event.agent.id).send("Try again.");
    }
  });
...
Using `shellCommand` from the same [helper file](https://github.com/getpaseo/paseo/blob/v0.8.0/plugin-examples/lifecycle-actions/server/inspect.ts):
...
export default function contribute(server: PluginServerContext) {
  server.on("agent.permission_requested", async (event, context) => {
    ...
    const agent = context.otto.agents.ref(event.agent.id);
...
**Shared payload shapes** (`@otto-code/plugin/server`):
...
Creation request
  ...
  → derive launch configuration with Otto runtime tools and daemon prompt
...
| Contract                           | Behavior                                                                                                  |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `context.otto`                     | Existing SDK connected to this daemon                                                                     |
...
| Plugin                                                                                                   | Includes                                                                     |
| -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| [lifecycle-logger](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/lifecycle-logger)       | All eleven hooks; JSON logs with environment values redacted                 |
| [lifecycle-actions](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/lifecycle-actions)     | Follow-ups, permissions, environment, provider switching, worktree selection |
| [agent-configuration](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/agent-configuration) | MCP injection and Codex sandbox/approval options                             |

Read logger output with `otto plugin logs lifecycle-logger` or the host's `daemon.log`.
...
```tsx
import type { PluginSurfaceProps } from "@otto-code/plugin/client";
...
| Field        | Meaning                                                                                                                      |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| `theme`      | Typed `PluginTheme` color tokens for the active Otto theme.                                                                  |
...
Otto owns the route, header, close action, host picker, error boundary, and query client. The plugin owns the surface body.

## Host UI

Import Otto-owned UI from `@otto-code/plugin/client/react-native` in client code. This example
opens a controlled modal, renders a host icon, and confirms the action with a toast:

```tsx
import type { PluginSurfaceProps } from "@otto-code/plugin/client";
import { Icon, Modal, useToast } from "@otto-code/plugin/client/react-native";
...
| `style`                 | `StyleProp<ViewStyle>` | None               | Styles the full body viewport, including empty space. Use for backgrounds.        |
...
Modal children keep the plugin runtime context. `useOtto`, `useRpc`, `useWorkspace`, and
`useAgent` work inside them.

### Scrolling

Import `ScrollView` and `FlatList` from `@otto-code/plugin/client/react-native` when content can appear in a
Otto modal. They accept React Native props and refs and integrate with the sheet's gestures. Outside
...
```tsx
import { FlatList, Modal } from "@otto-code/plugin/client/react-native";
...
```tsx
import { copyText, useToast } from "@otto-code/plugin/client/react-native";
...
for long-press selection and OS Copy. Import `TextInput` from `@otto-code/plugin/client/react-native` for modal forms. It accepts React Native
...
The runnable [modal UI example](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/modal-ui)
...
`Icon` renders a [Lucide icon](https://lucide.dev/icons/) from Otto's installed icon set. Plugin bundles do not import
...
registrations are client contributions. Otto applies the transformer while building the render
model, including every live streaming update.

```tsx
import type { PluginClientContext, PluginTimelineItemProps } from "@otto-code/plugin/client";
import { Text } from "react-native";
import { z } from "zod";
...
Each replacement may set an optional plugin-local `id`; otherwise Otto uses its index within that
source item's output.

Renderers receive `agentId`, `item`, `timestamp`, `theme`, `host`, and `layout`. Otto validates
`item.data` with the registered schema before rendering. Keep transformers synchronous and
deterministic. Otto memoizes results by source-item reference and derives replacement identity from
the source row, so updates to one streaming item do not remount its renderer. Use the exported
`useRevealedText(text, phase)` hook when a renderer should pace streaming text like Otto's built-in
assistant rows.
...
async function publishReview(agentId: string, { otto }: PluginHandlerContext) {
...
same plugin and `id`. If its renderer is missing, Otto shows the existing unavailable row. Daemons
...
Plugin UI runs on desktop, browser, iOS, and Android, across every Otto theme. `theme` is a typed `PluginTheme` mapped from the active host theme. Color and spacing must come from those props. Hardcoded colors and unstyled `Text` break when the host theme changes.
...
Every color is a hex string; anything else fails to load. Otto expands the palette into the full
token set the matching built-in appearance uses, so a contributed theme covers panels, menus, diffs, status
...
| `ring`            | Extra-muted text and raised-surface tint; terminal bright black in dark themes |

Otto derives focus feedback from the accent colors and retains its built-in scrollbar palette.

`appearance` is `"light"` or `"dark"`. Otto uses it to select the matching surface, status,
...
later disabled or removed, Otto falls back to the default theme rather than leaving the app
unpainted.
...
`index.client.tsx`. It appears under **Settings → Host → Plugins → your plugin** on that host.
...
The component receives `PluginSurfaceProps`. Otto owns the header, back navigation, safe areas,
...
Import settings components from `@otto-code/plugin/client/ui`. They work with your own state and RPCs;
no form wrapper or storage binding is required.

```tsx
import { useState } from "react";
import { SettingsCard, SettingsSection, SettingsSwitch } from "@otto-code/plugin/client/ui";
...
| `SettingsSelect`                   | Row props plus required string `value`, `options: { label, value }[]`, `onValueChange`; optional `disabled`. Uses Otto's adaptive menu. |
...
```ts
import { defineSettings } from "@otto-code/plugin";
import { z } from "zod";
...
See the complete [settings example](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/settings)
...
```tsx
import { type PluginAgentPanelProps, useAgent, useWorkspace } from "@otto-code/plugin/client";
import { useMemo } from "react";
import { Text, View } from "react-native";
...
Read cached state with `useWorkspace(workspaceId, selector)` and `useAgent(agentId, selector)`. A selector is required. Otto compares its result shallowly, so selecting `{ name, status }` does not re-render when unrelated fields change. Select every field the component renders in one call; do not select the whole snapshot.
...
Otto owns tab focus, splitting, closing, persistence, query state, the API/RPC providers, and the render error boundary. A restored tab whose plugin, panel, context, workspace, or agent is unavailable stays open with an unavailable message instead of crashing the workspace.
...
client.addCommandCenterItem({
  ...
  keywords: ["inspect"],
  context: "agent",
  async onSelect({ otto, rpc, workspace, agent, openPanel }) {
    await otto.workspaces.ref(workspace.id).setTitle(`Review ${agent.id}`);
...
| `otto`                    | All                 | Selected host's existing `OttoApi`.                                                                             |
...
An agent callback may open either an agent panel or a workspace panel. A workspace callback may open only a workspace panel. Unknown surface and panel IDs fail visibly. Use `otto` for normal workspace, agent, provider, and daemon-config operations. Use `rpc` for plugin-specific filesystem, credential, vendor, or daemon-local work.

## Slash commands

Register a command that runs in the Otto client when the user submits `/name args` from the
...
`args` is `"src"`; Otto trims only the remainder's leading and trailing whitespace and leaves
parsing to the plugin. Otto owns the autocomplete row, input clearing, and the error toast. It
...
Try the [button example](https://github.com/getpaseo/paseo/tree/v0.8.0/plugin-examples/buttons) for
...
Compact header buttons use icons without labels or chevrons. Otto moves excess contributions
...
These contracts are exported from `@otto-code/plugin/client`.
...
An action runs on the client. Otto marks the button busy until its promise settles, blocks repeated
presses, and shows failures in a toast. A failed action can be retried. Use the client's `otto` for
...
Otto removes leading, trailing, and consecutive separators after filtering hidden items.
...
Render a React Native icon or indicator within the supplied size. Otto bounds the icon slot and
...
Render the body only; Otto owns anchoring, scrolling, padding, and sheet presentation. Content can
use `useOtto`, `useRpc`, `useWorkspace`, `useAgent`, and the installation's React Query cache.
...
`remove()` is idempotent. Updates after removal do nothing. Otto removes outstanding buttons when
...
Use `useOtto()` for ordinary Otto operations from a surface. It borrows the selected host's existing connection; do not create another client.

```tsx
import { type PluginSurfaceProps, useOtto } from "@otto-code/plugin/client";
...
The returned API covers projects, workspaces, agents, terminals, providers, and daemon config. See the [SDK API reference](/docs/sdk/reference) for its methods. Connection lifecycle methods are intentionally absent because Otto owns the connection.

## Add plugin-specific backend behavior

Use plugin RPC only for work that is not a normal Otto operation: reading a vendor API, accessing daemon-local resources, or keeping credentials off the client.
...
Backend handlers receive the same `OttoApi` as `{ otto }`. Their connection belongs to the subprocess and closes when the plugin stops. It does not subscribe to timelines or catalog events until plugin code subscribes. Follow the [SDK event contract](../../sdk/events.md) for cleanup and timeline replacements. Backend code can use Node APIs and dependencies installed in the plugin directory.
...
Otto adds `[otto]` entries when the plugin starts loading, becomes ready, starts stopping, and has
stopped. It records compilation and load failures as stderr entries, including failures that happen
before the plugin subprocess starts. Otto also captures output emitted during initialization, RPC
...
otto --host <url> plugin logs my-plugin
```
...
Otto retains up to 500 entries and 256 KiB per plugin in memory. Individual lines are capped at
...
also written to the daemon log at `$OTTO_HOME/daemon.log`.
...
Otto owns the composer menu, search picker, selected pill, draft state, and submission. The `text` value is the complete snapshot sent to the agent.

## Hosts and lifecycle

Plugins are installed per daemon. When the same contribution exists on several connected hosts, Otto shows one sidebar item and adds a host picker. The selected host supplies the bundle, Otto API, RPC transport, and query cache. Calls never fall through to another host when the selected host is offline.
...
A failed installation remains configured. Correct the source or API requirement and run
`otto plugin reload <id>` to load it again. The replacement registrations restore available
surfaces, panels and settings screens. Reload tears down active provider sessions; reopening one
requires that provider's [persistence contract](/docs/plugins/v0.8/providers#add-persistence-and-replay).
Recovery does not retry an interrupted turn.
...
otto plugin add https://git.example.com/owner/repository.git --ref main
otto plugin add owner/monorepo:plugins/review
otto plugin ls [id]
otto plugin update <id>
...
Use `update` when you want Otto to contact a tracked Git remote and install an available update.
...
> **Trust every plugin you add.** `otto plugin add` and `otto plugin install` mean “I trust this codebase.” Server code and Git preparation commands run unsandboxed with the daemon user's access on the daemon host; client contributions run inside Otto. Dependencies and future updates are part of that decision. With the global `--host` option, commands run on the remote daemon host.
...
that Otto does not provide, generate source or assets, or perform another required preparation
step:
...
`build` is a list of non-empty argv arrays. Otto runs each executable directly, without a shell,
...
The switch is the root `pluginsEnabled` field in `config.json`. After changing it, run `otto reload --json`. Enabling starts every configured plugin whose own `enabled` value is not `false`; disabling tears down all plugins. No daemon restart is required. Manual edits to plugin source entries are not reloaded; use the plugin lifecycle commands for those.

## Load failures

Use `otto plugin ls` to read the current status and error.

| Symptom                                                               | Check                                                                                                                                   |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `This plugin uses an older Paseo plugin API`                          | The directory has only an `index.ts` entry. Follow the [migration guide](/docs/plugins/v0.8/migration).                                 |
...
| Edited code does not appear                                           | Run `npm run typecheck`, then `otto plugin reload <id>`.                                                                                |
| Reload fails                                                          | Read `otto plugin ls` and `otto plugin logs <id>`, fix the source error, then reload; Otto does not restore the previous bundle.        |
| Plugin exits unexpectedly                                             | Read `otto plugin logs <id>` for retained initialization, cleanup, stderr, and final crash output.                                      |
