...
> **For the Paseo v0.8 API integrated into Otto.** The v0.7 guide is historical; the current Otto runtime requires the split entries.
...
7. Keep `otto-plugin.json`, `package.json`, and `tsconfig.json` at the root.
8. Delete the old root entry. Otto does not load it.
...
| `import { defineRpc, defineAttachmentSource } from "@otto-code/plugin/server"` in shared files | `import { defineRpc, defineAttachmentSource } from "@otto-code/plugin"`                                      |
| `ZodOutput<typeof contract.input>` handler parameter types                                     | `RpcInput<typeof contract>` from `@otto-code/plugin`; `RpcOutput` for return types                           |

Import `PluginClientContext` from `@otto-code/plugin/client` and `PluginServerContext` from
`@otto-code/plugin/server`. Remove imports of the old context type. Client registrations return idempotent removal functions, except header buttons and composer pills, which return `{ update, remove }` handles. Preserve any remover the plugin calls before teardown; Otto removes outstanding
registrations after the entry cleanup runs.

### Composer pills

Update the plugin project's `@otto-code/plugin` dependency, then run `npm run typecheck`.
...
Move hooks (`useOtto`, `useRpc`, `useSettings`, `useAgent`, `useWorkspace`) and client contribution
types from `@otto-code/plugin` to `@otto-code/plugin/client`. Move `Icon` to
`@otto-code/plugin/client/react-native`. Import server contexts and lifecycle contracts from
`@otto-code/plugin/server`. Shared helpers (`defineRpc`, `defineSettings`, `defineAttachmentSource`),
...
| Old entry                        | 0.8 entry                               |
| -------------------------------- | --------------------------------------- |
| `@otto-code/plugin/react-native` | `@otto-code/plugin/client/react-native` |
| `@otto-code/plugin/ui`           | `@otto-code/plugin/client/ui`           |
| `@otto-code/plugin/provider`     | `@otto-code/plugin/server/provider`     |
| `@otto-code/plugin/acp`          | `@otto-code/plugin/server/acp`          |

The old entries and the pre-0.8 `@paseo/plugin` scope are removed. `/client/host` is private to
Otto's app integration and is never a plugin-author import.
...
| `This plugin uses an older Paseo plugin API`                                                                               | The directory still has only the old root entry. Create a runtime entry, move registrations, then delete the old file.    |
...
## 7. Declare the Paseo API requirement

After migrating the entries and imports, add the minimum runtime version to `otto-plugin.json`:
...
Keep your existing ID and build commands. Otto accepts either `otto-plugin.json`
or an upstream `paseo-plugin.json`, never both in the same directory.
Missing `requirements.paseo` means `<0.8.0` and is rejected by the current runtime.
Adding the field alone does not migrate the entrypoints or imports.

The current integrated Paseo API is 0.8.0. This version is separate from Otto's
package version: use a matching Otto `@otto-code/plugin` development dependency
for typechecking and the required Paseo API range in the manifest. The corresponding
`@getpaseo/plugin` split entries remain runtime aliases for upstream-authored code;
they do not reinstate old mixed entries. See [requirements](reference#requirements).
