...
description: Local plugin files, contributions, Otto SDK access, RPCs, attachments, logs, lifecycle, hosts, and CLI commands.
nav: Reference
order: 46
category: Plugins
---
...
The generated declaration file supplies `@otto-code/plugin` and `@otto-code/plugin/server` types for local typechecking. Otto supplies the runtime modules. Regenerate a fresh project with the matching CLI when the plugin contract changes.
...
| `@otto-code/plugin/server` | `defineRpc`, `defineAttachmentSource`, and handler types |

Otto rejects imports from `*.server` files into client modules and imports from `*.client` files into server modules. Keep shared modules free of Node and React Native runtime code.
...
```tsx
import type { PluginSurfaceProps } from "@otto-code/plugin";
...
Otto owns the route, header, close action, host picker, error boundary, and query client. The plugin owns the surface body. Icons use [Lucide](https://lucide.dev/icons/) names.

## Theme and layout

Plugin UI runs on desktop, browser, iOS, and Android, across every Otto theme. `theme` is a typed `PluginTheme`. Color and spacing must come from those props. Unstyled `Text` is black and fails in dark themes.
...
Client code can import `react`, `react-native`, `@tanstack/react-query`, `zod`, `@otto-code/plugin`, and `@otto-code/plugin/server`. Install them locally for typechecking; Otto provides the client runtime instances.
...
```tsx
import { type PluginAgentPanelProps, useAgent, useWorkspace } from "@otto-code/plugin";
import { useMemo } from "react";
import { Text, View } from "react-native";
...
The returned API covers workspaces, agents, providers, and daemon config. See the [SDK API reference](/docs/sdk/reference) for its methods. Connection lifecycle methods are intentionally absent because Otto owns the connection.
...
Backend handlers receive the same `OttoApi` as `{ otto }`. Their connection belongs to the subprocess and closes when the plugin stops. Backend code can use Node APIs and dependencies installed in the plugin directory.
