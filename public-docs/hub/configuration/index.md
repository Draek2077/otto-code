...
[single-repo-team-bot](https://github.com/Draek2077/hub/tree/main/examples/single-repo-team-bot) is a complete bundle in this shape: Discord, Slack, and GitHub workflows running a classifier and a worker on shared partials. Copy `.otto/` into your repository and replace the placeholders its README lists.

## Generated starter bundle

`otto hub init`, and the guided setup that interactive `otto hub login` offers, write two files for the directory you run them in:
...
- **GitHub source**: the complete `.otto` bundle on the repository's default branch.
...
otto hub deploy -p my-project --dry-run
otto hub deploy -p my-project
```

Both commands discover `.otto/hub.yml`, every direct `.otto/workflows/*.yml` file, and each referenced file below `.otto/workflows/partials/`. Files are sent in deterministic path order through the same bundle request. Dry-run calls server-side validation and does not create or activate a revision.
