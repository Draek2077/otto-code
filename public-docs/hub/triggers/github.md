...
`.otto/workflows/github-change.yml`:
...
On comment events, Hub reacts with 👀 when accepted, 🚀 when the agent starts, 👍 on completion, and 👎 on failure. `${{ otto.prompt }}` contains normalized request text, not event identifiers. Use `${{ otto.context }}` in prompt text when the step explicitly needs provider context.
