...
```sh
otto hub login https://your-hub.example.com
otto hub projects
otto hub connect
```
...
`.otto/hub.yml` names project-wide resources:
...
`.otto/workflows/github-help.yml` contains one trigger and its ordered steps:
...
`daemon` is the daemon slug shown by Hub. `cwd` is a directory on that machine. `${{ otto.prompt }}` is the normalized request text after the provider marker and declared input headers are removed.

Workflow files are discovered as direct `.yml` children of `.otto/workflows/`. You do not list them in `hub.yml`.
...
```sh
otto hub deploy -p your-project --dry-run
otto hub deploy -p your-project
```
...
Logging out does not disconnect the daemon. When the daemon is connected to the same Hub as the active CLI login, use `otto hub logout --disconnect-daemon` to remove both relationships.

## Next

[single-repo-team-bot](https://github.com/Draek2077/hub/tree/main/examples/single-repo-team-bot) is a complete bundle covering all three providers, with a classifier, a worker, and shared prompt partials.
