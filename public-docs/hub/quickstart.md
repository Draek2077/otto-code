...
You need [Otto installed and running](/docs), Node.js, and a Slack workspace where you can create an app.
...
```sh
otto hub login http://localhost:3000
```
...
If `.otto/` already exists, setup asks before replacing it. Declining the daemon connection prints `otto hub connect <hub>; then otto hub init` — both commands, because connecting alone does not create the workflow. Declining only the starter workflow prints `otto hub init`.
...
Hub keeps its local state in your user data directory, normally `~/.local/share/otto-hub`. [Self-hosting](/docs/hub/self-hosting) covers deployment and advanced configuration when you outgrow the local run.
