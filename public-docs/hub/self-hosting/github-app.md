...
Replace `hub.example.com` with your `OTTO_HUB_APP_URL`.
...
| ------------- | ------------ | ----------------------------------------- |
| Contents      | Read & write | Read the `.otto` bundle, let agents push  |
...
Push is what makes configuration sync work. Without it, Hub never learns that the `.otto` bundle changed.
...
The connection appears with a slug derived from the account: an installation on `Draek2077` becomes `Draek2077-github`. That slug is how configuration names this connection.
...
- **Configuration sync.** Any repository in the installation can hold the canonical `.otto` bundle. See [Configuration](/docs/hub/configuration).
- **Tokens.** Scoped, per-execution GitHub credentials.

Which repositories the installation covers is a GitHub setting. Change it on GitHub, not in Otto.
