...
description: Deploy Otto Hub with PostgreSQL and a public HTTPS origin, using Docker Compose or Fly.
nav: Self-hosting
order: 74
category: Hub
---
...
| Variable               | Purpose                                                                      |
| ---------------------- | ---------------------------------------------------------------------------- |
| `OTTO_HUB_APP_URL`     | Public origin used by the dashboard, authentication, callbacks, and webhooks |
| `OTTO_HUB_AUTH_SECRET` | Protects browser sessions and derives execution credentials                  |
| `DATABASE_URL`         | PostgreSQL connection string                                                 |

Generate `OTTO_HUB_AUTH_SECRET` once and keep it across restarts:
...
The password must be at least 12 characters. Sign in with it once, replace it in the dashboard, then remove `OTTO_BOOTSTRAP_OWNER_PASSWORD` from the deployment. Hub keeps the account and organization.
...
Set `OTTO_HUB_APP_URL`, `OTTO_HUB_AUTH_SECRET`, and the three bootstrap values in `.env`, then run:
...
fly secrets set -a your-hub \
  OTTO_HUB_AUTH_SECRET="$(openssl rand -hex 32)" \
  OTTO_BOOTSTRAP_ORGANIZATION="My organization" \
  OTTO_BOOTSTRAP_OWNER_EMAIL=me@example.com \
  OTTO_BOOTSTRAP_OWNER_PASSWORD=replace-with-a-temporary-password
```
...
Slack and GitHub call Hub at `OTTO_HUB_APP_URL`:

| Provider setting             | URL                                                   |
| ---------------------------- | ----------------------------------------------------- |
| GitHub webhook               | `<OTTO_HUB_APP_URL>/webhook`                          |
| GitHub OAuth callback        | `<OTTO_HUB_APP_URL>/api/integrations/github/callback` |
| Slack Events API request URL | `<OTTO_HUB_APP_URL>/api/integrations/slack/events`    |
| Slack OAuth callback         | `<OTTO_HUB_APP_URL>/api/integrations/slack/callback`  |
...
A Hub on `localhost` or a LAN address still runs manual and daemon workflows, but Slack and GitHub cannot reach it. To test provider setup locally, point `OTTO_HUB_APP_URL` at an HTTPS tunnel before starting Hub. A changed tunnel origin requires updated provider settings and a Hub restart.
