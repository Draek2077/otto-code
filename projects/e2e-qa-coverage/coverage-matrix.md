...
| Offline project survives reload, rejects a missing destination, and reconnects its workspace and chat | ✅     | `project-offline-relocation.spec.ts`                                   | T1   | -                                                 |
...
| Sidebar workspace pin shortcut and pinned-chat behavior                                               | 🟡     | `sidebar-workspace-pin-shortcut.spec.ts`                               | T1   | tab-launcher pins removed; the daemon RPC remains |
| Workspace tab row / rail, overflow, creation, maximize, and focus-mode exit                           | 🟡     | `workspace-focus-mode.spec.ts`, `launcher-tab.spec.ts`                 | T1   | add explicit dual-orientation coverage            |
...
| Detached row stays fixed through virtualized bottom mutations                          | ✅     | `turn-completion-scroll.spec.ts`                                                    | T1   | -   |
...
| Explorer pane-host dock (defaults, toggle, shortcuts, placement, upgrade migration, and drag stability)                | ✅     | `explorer-sidebar.spec.ts`, `changes-shortcut-placement.spec.ts`, `empty-pane-keyboard.spec.ts`, `explorer-pane-placement.spec.ts`, `explorer-surface-upgrade.spec.ts` | T1   | -   |
...
| Behavior                                                                                      | Status | Specs / plan                                | Tier | Pri |
| --------------------------------------------------------------------------------------------- | ------ | ------------------------------------------- | ---- | --- |
| Changes options open and reuse the Git Commit log tab                                         | ✅     | `changes-commit.spec.ts`                    | T1   | -   |
| Manual Conventional Commits, checked-file scope, Commits history, and hook-failure retry      | ✅     | `changes-commit.spec.ts`                    | T1   | -   |
| Branch switcher                                                                               | ✅     | `branch-switcher.spec.ts`                   | T1   | -   |
| PR pane (GitHub fixtures: `helpers/github-fixtures.ts`)                                       | ✅     | `pr-pane.spec.ts`                           | T1   | -   |
| Diff canvas single-surface rendering, typography, and wrapped-line overflow                   | ✅     | `changes-pane.spec.ts`                      | T1   | -   |
...
| Behavior                                                                                           | Status | Specs / plan                                                                   | Tier  | Pri |
| -------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------ | ----- | --- |
| Workspace artifact discovery across clients, failed-add feedback, and local dismissal after reload | 🟢     | `artifact-workspace-discovery.spec.ts`                                         | T1    | P0  |
