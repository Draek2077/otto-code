...
| [markdown-editing](markdown-editing/markdown-editing.md)                                        | Charter  | Otto renders markdown better than MarkText does and edits it worse than Notepad does. Build the missing half: `@codemirror/lang-markdown` at the `buildLanguageExtension` seam (list continuation, GFM), formatting commands behind a new **`markdown-editor` focus scope** (because `Mod+B` is already Go to definition), a touch-first formatting toolbar, headings in `extractSymbols`, then paste-HTML-as-markdown and daemon-side image drop, then **live preview as CM6 decorations** (not a second engine, per the one-engine rule in [docs/text-editor.md](../docs/text-editor.md)) with focus and typewriter modes, then the File-rendering parity rows below (KaTeX, alerts, footnotes, HTML tables, interactive checkboxes), then HTML/PDF export. Mobile is first-class: a phone has no chords, so the toolbar is the only affordance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
