...
| [markdown-editing](markdown-editing/markdown-editing.md)                                        | Partial  | Otto renders markdown better than MarkText does and edited it worse than Notepad did. **Built (Phase 1):** `@codemirror/lang-markdown` at the `buildLanguageExtension` seam, so Enter continues lists, Backspace deletes one level of markup, GFM tables/task lists parse, and pasting a URL over a selection makes a link; fences resolve their grammar through `filenameForHoverLanguage` rather than a second name-to-extension map. Formatting is a set of **pure transforms** (`editor/markdown/markdown-format.ts`, 37 unit tests) wrapped as CM6 commands that DECLINE outside markdown context, which is what lets one keymap serve `Mod+B` as bold in a `.md` file and Go to definition in a `.ts` one. A new **`markdown-editor` focus scope** carries the combos that collide with globals (`Mod+K` is a link here, the command center everywhere else) and INHERITS `code-editor`, so Save/Find/Go-to-line keep working; `bindingSpecificity` gained a third rank for that. Touch-first **formatting toolbar** (16 commands, horizontally scrollable, the only affordance on a phone) plus 14 new Material Symbols and all 8 locales. Headings extract client-side (`extractMarkdownHeadings`) because `CodeSymbolKindSchema` is a wire enum an old client would reject. **Open:** Phase 1 tail (outline sheet not yet sourcing markdown headings, no browser test for the keymap fall-through), then Phases 2-5 — paste-HTML-as-markdown, daemon-side image drop, table editing, link autocomplete; **live preview as CM6 decorations** with focus/typewriter modes; the File-rendering parity rows below (KaTeX, alerts, footnotes, HTML tables, interactive checkboxes); HTML/PDF export                                                                                                                                                                                                       |
...
_Five rows below are **claimed by [markdown-editing](markdown-editing/markdown-editing.md) Phase 4**
and will be built there, not separately: math (KaTeX), GitHub alerts, footnotes, HTML `<table>`, and
interactive checkboxes. A live-preview editor that cannot render math is not a live-preview editor,
so they stopped being optional._
