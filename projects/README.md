...
and are built there, not separately. **All five have shipped on web and desktop**: GitHub alerts,
HTML `<table>`, footnotes, interactive checkboxes and math. Math renders through KaTeX to **MathML**,
which every browser lays out natively, so it needs no stylesheet and the HTML export shows formulae
with no script either. **Native still shows the TeX source** — KaTeX emits only HTML and MathML and
React Native renders neither, so a formula there needs a WebView carrying a bundled KaTeX, the
pattern `mermaid-diagram.native.tsx` already established._
