...
| [brain-coding-capabilities](brain-coding-capabilities/brain-coding-capabilities.md)             | In build | Coding-specific functionality on `@otto-code/brain` so a local model reaches hosted-provider parity. Four tracks; **the bench is the spine** (`rankModels` measures coding quality but nothing routes on it yet). **In flight:** A — wire the dormant SWE-bench harness (`mine.ts`/`repo.ts`) into a real tool-using, test-scored Task; D — feed tool-result images to the model so browser-verify stops being blind on openai-compat (`ottoResultToText` drops image parts); B1 — carry catalog coding metadata onto scanned models. **Next:** B2 (ranking-driven VRAM-aware routing, needs A). **Parked:** C (local FIM/autocomplete) |
| [brain-host-control](brain-host-control/brain-host-control.md)                                  | Charter  | The resident local-brain's **control surface**. Built-in HTTPS + API-key already retired the `otto-brain-relay`'s TLS/auth job inside `@otto-code/brain` (`config.tls`, `otto brain start/stop/restart/status`); what remains is daemon-managed brain lifecycle plus start/stop from Otto's Settings and its existing cross-platform Electron tray. The brain does not grow its own tray — Otto is the desktop presence; the CLI stays the headless quit path                                                                                                                                                                           |
