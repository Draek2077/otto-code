...
3. **In User mode** the workspace is chat-first: agent tabs, the composer, personalities, artifacts, and the browser pane. No tab-strip terminal/preview catalog, no explorer sidebar, no git buttons, no diff badges, no file tabs, no keyboard shortcuts into hidden surfaces. Copy leans plain-language.
...
| Surface                                                                                                           | Where                                                                                                                                                     | Gate                                                                                                                                       |
| ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Explorer sidebar (Changes / Files / Search / PR)                                                                  | `components/explorer-sidebar.tsx`, `compact-explorer-sidebar-host.tsx`, mounted in `workspace-screen.tsx` (`showExplorerSidebar`)                         | Extend `shouldShowWorkspaceExplorerSidebar`; also hide the explorer toggle + source-control buttons and the `DiffStat` badge (~L3592–3639) |
...
| Dev keyboard shortcuts: `sidebar.open.{files,search,changes}`, `workspace.terminal.new`, `workspace.pane.split.*` | `keyboard/keyboard-shortcuts.ts`                                                                                                                          | Gate at action dispatch (one place), not per-binding; shortcuts screen greys them out                                                      |
