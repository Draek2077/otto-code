...
`SyncedLoader` spinning is the thing that makes the hero feel alive — so the replacement must move
...
- **Commit messages are plausible and well-formed.** They appear in Git Log, blame and file-history
  shots.
- **No lorem ipsum, no placeholder copy** anywhere that renders in the browser pane — §2 photographs
...
`—` in the scenario column means no producing scenario exists yet.

| Shot id              | §   | Kind  | Producing scenario                            |
| -------------------- | --- | ----- | --------------------------------------------- |
| `hero-desktop`       | —   | full  | `00-website-hero`                             |
...
| `team-runs`          | 3   | focus | `23-orchestration-runs` —                     |
| `team-memory`        | 3   | focus | `23-orchestration-runs` —                     |
| `auto-artifacts`     | 4   | focus | `13-artifacts` —                              |
| `auto-artifact-tab`  | 4   | focus | `13-artifacts` —                              |
| `auto-schedules`     | 4   | focus | `14-schedules` —                              |
| `auto-suggested`     | 4   | focus | `09-composer-intelligence`                    |
| `ide-definition`     | 5   | focus | `19-editor-ide` —                             |
| `ide-diagnostics`    | 5   | focus | `24-code-intelligence` —                      |
| `ide-rename`         | 5   | loop  | `24-code-intelligence` —                      |
| `ide-solution`       | 5   | focus | `24-code-intelligence` —                      |
| `ui-widget`          | 6   | focus | `22-widgets` —                                |
| `ui-tasklist`        | 6   | focus | `01-agent-live`                               |
| `ui-mermaid`         | 6   | focus | `22-widgets` —                                |
| `ui-themes`          | 6   | focus | `12-themes` — (triptych, 3 themes)            |
| `cost-context`       | 7   | focus | `20-context-cost` —                           |
| `cost-ledger`        | 7   | focus | `20-context-cost` —                           |
| `cost-metrics`       | 7   | focus | `20-context-cost` —                           |
| `ship-diff`          | 8   | focus | `03-diff-review`                              |
| `ship-commit`        | 8   | focus | `10-diff-ai-review` —                         |
| `ship-blame`         | 8   | focus | `25-git-history` —                            |
| `ship-worktree`      | 8   | focus | `18-worktrees` —                              |
| `model-local`        | 9   | focus | `17-multi-provider` —                         |
| `model-local-verify` | 9   | loop  | `02-preview-verify`, `DEMO_PROVIDER=local-ai` |
| `voice-mode`         | 10  | loop  | `21-voice` —                                  |
| `voice-playback`     | 10  | focus | `21-voice` —                                  |
| `paseo-panes`        | 11  | focus | `15-workspace-layouts` —                      |
...
deployed until at least the hero and §1–§3 have real assets.
