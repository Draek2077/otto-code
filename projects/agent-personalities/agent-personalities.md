...
## Shipped default roster (starter team)

Added after the core feature landed: a shipped **starter team** so a fresh host opens with a working, role-complete roster instead of an empty editor. One source of truth — `packages/protocol/src/default-personalities.ts` `DEFAULT_AGENT_PERSONALITIES` — imported by both the daemon (first-run seeding) and the app (restore button). Six personalities, all Claude (the launch assumption), covering all 7 roles with a mix of single- and multi-role setups, 2 men / 3 women / 1 robot:

| Name     | Roles                 | Model            | Effort · Mode        | Voice (Kokoro v1.0) | Glow        |
| -------- | --------------------- | ---------------- | -------------------- | ------------------- | ----------- |
| Atlas    | orchestrator, chatter | claude-opus-4-8  | high · default       | am_onyx             | indigo→gold |
| Sage     | advisor               | claude-opus-4-8  | xhigh · plan         | af_heart            | teal→violet |
| Vera     | judger                | claude-sonnet-5  | high · plan          | bf_emma             | rose→amber  |
| Pixel    | artificer             | claude-sonnet-5  | medium · acceptEdits | af_nova             | pink→cyan   |
| Dash     | worker, scheduler     | claude-haiku-4-5 | low · auto           | am_puck             | green→lime  |
| Sprocket | chatter, worker       | claude-sonnet-5  | medium · default     | am_echo             | chrome→blue |

- **Model choice by cost/fit:** Opus for low-volume high-stakes reasoning (orchestration, advice); Sonnet for everyday building/review; Haiku for fast, cheap, high-volume/recurring unattended work.
- **Voices are Kokoro v1.0 (kokoro-multi-lang-v1_0) names — a soft binding.** On a host running OpenAI TTS or the older Kokoro v0.19 they degrade to the host default at playback; nothing to fix. On a host without Claude the whole roster shows "out of commission" until a matching provider exists.
- **Seeding (first-run + delete-safe):** `bootstrap.ts` seeds the in-memory roster when the persisted `agents.agentPersonalities` section is **absent** (undefined, distinct from an empty roster the user cleared), then `DaemonConfigStore.seedDefaultPersonalitiesIfAbsent` records the seed on disk once — writing **only** the personalities branch so unrelated defaults (speech, etc.) aren't frozen onto disk. Once the section exists on disk (even empty), seeding is a permanent no-op, so **deleting the whole team sticks across restarts.**
- **Restore button:** the editor (`agent-personalities-section.tsx`) shows "Add starter team" in the empty state and "Restore starter team (N missing)" as a footer when some builtins are absent. Restore re-adds only builtins whose stable `personality_builtin_*` id is missing, so kept/renamed ones never duplicate.
- **Guardrail tests:** `server/agent/default-personalities.test.ts` validates every default against the live Claude model manifest, Claude mode set, Kokoro v1.0 voice catalog, effort scale, role coverage, and name handle rules; `daemon-config-store.test.ts` covers first-run seed, no-reseed-when-present, and delete-stays-deleted.
