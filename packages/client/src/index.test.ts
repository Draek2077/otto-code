...
import { createOttoApi, createOttoClient } from "./index.js";
import { DaemonClient } from "./daemon-client.js";
import type { OttoAgent, OttoClient, OttoWorkspace } from "./index.js";
...
async function connectClient(
...
): Promise<{ client: OttoClient; ws: FakeWebSocket }> {
  vi.stubGlobal("WebSocket", FakeWebSocket);
  const client = createOttoClient({
    url: "ws://daemon.test",
  ...
  expect(hello.clientId).toEqual(expect.stringMatching(/^otto-sdk-/));
  ws.message(
    sessionMessage({
      type: "status",
      payload: {
...
function createWorkspace(input: Partial<OttoWorkspace> = {}): OttoWorkspace {
  return {
    id: "workspace_sdk",
...
function createAgent(input: Partial<OttoAgent> = {}): OttoAgent {
  return {
    id: "agent_sdk",
    provider: "codex",
    cwd: "/repo/sdk",
...
test("createOttoClient exposes workspace list through the daemon client", async () => {
...
test("createOttoApi borrows daemon capabilities without exposing connection ownership", () => {
  ...
  const otto = createOttoApi(daemonClient);

  expect(Object.keys(otto).sort()).toEqual(["agents", "config", "providers", "workspaces"]);
  expect("connect" in otto).toBe(false);
  expect("close" in otto).toBe(false);
  expect("skills" in otto.agents).toBe(false);
});
