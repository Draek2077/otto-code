...
export async function createSkillsSandbox(
  ...
  const bundledSkills = options.bundledSkills ?? ["otto", "otto-advisor", "otto-loop"];
  const root = await mkdtemp(path.join(os.tmpdir(), "otto-e2e-skills-"));
...
export async function serveRealSkillsCommands(page: Page, sandbox: SkillsSandbox): Promise<void> {
  ...
  await page.addInitScript((commands: readonly string[]) => {
    const bridge = (window as unknown as { ottoDesktop?: { invoke: unknown } }).ottoDesktop;
    ...
    bridge.invoke = (command: string, args?: Record<string, unknown>) =>
      skillCommands.has(command)
        ? window.__ottoSkillsInvoke!(command, args ?? null)
        : inner(command, args);
  }, SKILL_COMMANDS);
}
