...
declare global {
  interface Window {
    __ottoStartupPresentationTrace?: StartupPresentation[];
  }
}
...
async function observeStartupPresentation(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    window.__ottoStartupPresentationTrace = trace;
...
async function getStartupPresentation(page: Page): Promise<StartupPresentation[]> {
  return page.evaluate(() => window.__ottoStartupPresentationTrace?.slice() ?? []);
}
