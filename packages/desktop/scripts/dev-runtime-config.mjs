...
export function resolveDevRuntime(env = process.env) {
  const remoteDebuggingPort = env.OTTO_ELECTRON_REMOTE_DEBUGGING_PORT
    ? parsePort(env.OTTO_ELECTRON_REMOTE_DEBUGGING_PORT, "OTTO_ELECTRON_REMOTE_DEBUGGING_PORT")
    : 0;
  return {
    electronFlags: buildElectronFlags(env.OTTO_ELECTRON_FLAGS, remoteDebuggingPort),
    userDataDir: resolveDevUserDataDir({
      devRoot: env.OTTO_DEV_ROOT,
      inheritedUserDataDir: env.OTTO_ELECTRON_USER_DATA_DIR,
      fallbackRoot: env.OTTO_DEV_RUNTIME_FALLBACK_ROOT,
    }),
  };
}
