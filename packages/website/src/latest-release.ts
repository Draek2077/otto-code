...
function pickWindowsAssets(assets: GitHubAsset[]) {
  const x64Suffixed = assets.find((asset) => /Otto-Setup-.*-x64\.exe$/.test(asset.name));
  const arm64 = assets.find((asset) => /Otto-Setup-.*-arm64\.exe$/.test(asset.name));
  const legacy = assets.find(
    (asset) =>
      /Otto-Setup-.*\.exe$/.test(asset.name) &&
...
export function getLatestAndroidVersionFromReleases(releases: GitHubRelease[]): string {
  const release = releases.find((candidate) => {
    ...
    return candidate.assets.some(
      (asset) => asset.name === `otto-${candidate.tag_name}-android.apk`,
    );
  });
