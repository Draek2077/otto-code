...
OTTO_HOME=~/.otto        # Runtime state directory (agents/, etc.)
OTTO_LISTEN=127.0.0.1:6868  # Listen address (host:port or /path/to/socket)
```

`OTTO_HOME` defaults to `~/.otto` and isolates runtime artifacts like `agents/`. `OTTO_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
OTTO_HOME=~/.otto-blue OTTO_LISTEN=127.0.0.1:7777 npm run dev
```
