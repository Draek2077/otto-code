...
async function main(): Promise<void> {
  const tempDir = mkdtempSync(path.join(tmpdir(), "otto-git-observation-measurement-"));
  ...
  runGit(repoDir, ["config", "user.email", "measurement@otto.local"]);
  runGit(repoDir, ["config", "user.name", "Otto Measurement"]);
  ...
  try {
    healthy = createMeasuredService({ repoDir, ottoHome });
    ...
    degraded = createMeasuredService({ repoDir, ottoHome, failWorktreeWatch: true });
    startGitCommandMetrics();
    ...
    const outputPath = process.env.OTTO_GIT_OBSERVATION_REPORT?.trim();
    if (outputPath) {
