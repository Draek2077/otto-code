...
export function resolveSupervisorLogFile(
  ...
  const envRotateSize = env.OTTO_LOG_ROTATE_SIZE?.trim();
  const envRotateMaxFiles = parseOptionalPositiveInteger(env.OTTO_LOG_ROTATE_COUNT);
  let logPath = path.join(ottoHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (configuredPath) {
    logPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(ottoHome, configuredPath);
  }
