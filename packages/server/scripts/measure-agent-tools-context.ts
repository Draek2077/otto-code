...
const MEASUREMENT_CWD = "/tmp/otto-agent-tools-measurement";
...
Measures the MCP tools/list payload for Otto agent tools.
...
async function measureCatalog(params: {
  ...
  const client = new Client({ name: "otto-agent-tools-measurement", version: "0.0.0" });
...
function formatMarkdown(measurements: CatalogMeasurement[], top: number): string {
  ...
  const delta =
    withoutBrowser && withBrowser
      ...
      : null;
  const lines: string[] = [];
  lines.push("# Otto Agent Tool Catalog Context");
  lines.push("");
