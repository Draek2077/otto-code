...
interface ClaudeCredentialRecord {
  ...
  /** Where the credentials were read from, so a refreshed token can be written
   *  back. Null when they came from the OS keychain rather than a file. */
...
export class ClaudeQuotaProvider implements ProviderUsageFetcher {
  ...
  private toCredentialRecord(
    ...
    return oauth?.accessToken
      ? { oauth: { ...oauth, accessToken: oauth.accessToken }, filePath }
      : null;
  }
