  ...
  it("treats a Codex HTML usage response as auth failure", async () => {
    ...
    fetchApi = mockFetch(
      new Map([
        ...
        // Otto refreshes on an auth failure, so the token endpoint is part of
        // this path and has to be mocked even though it is expected to fail.
...
describe("usage bars escalate as they fill", () => {
  ...
  beforeEach(() => {
    claudeHome = mkdtempSync(join(tmpdir(), "otto-tone-claude-"));
    codexHome = mkdtempSync(join(tmpdir(), "otto-tone-codex-"));
  });
...
describe("ClaudeQuotaProvider scoped weekly limits", () => {
  ...
  beforeEach(() => {
    claudeHome = mkdtempSync(join(tmpdir(), "otto-claude-limits-"));
  });
...
describe("ClaudeQuotaProvider scoped limit reconciliation", () => {
  ...
  beforeEach(() => {
    claudeHome = mkdtempSync(join(tmpdir(), "otto-claude-matrix-"));
  });
