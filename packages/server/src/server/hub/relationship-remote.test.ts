...
afterEach(async () => {
  ...
  await Promise.all(openOttoHomes.splice(0).map((home) => rm(home, { recursive: true })));
});
...
async function connectController(
  ...
  const ottoHome = await mkdtemp(path.join(tmpdir(), "otto-hub-socket-"));
  openOttoHomes.push(ottoHome);
