...
import type { AgentStreamEvent } from "@otto-code/protocol/agent-types";
import type { HubExecutionAgentCreateResponse } from "@otto-code/protocol/messages";
...
const RUN_REAL_HUB_POLICY = process.env.OTTO_REAL_HUB_POLICY === "1";
...
describe.skipIf(!RUN_REAL_HUB_POLICY)("Hub provider policy (real providers)", () => {
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "otto-codex-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "otto-codex-denied-"));
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "otto-claude-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "otto-claude-denied-"));
      const allowedFile = path.join(allowedRoot, "allowed.txt");
      const deniedFile = path.join(deniedRoot, "denied.txt");
      const providerOptions = {
