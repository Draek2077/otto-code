...
function candidate(input: {
  ...
  return {
    ...
    project: {
      projectKey: "key",
      projectName: input.projectName ?? "Draek2077/otto-code",
...
describe("scoreAgentHistoryCandidate", () => {
  ...
  it("matches the project name", () => {
    expect(
      scoreAgentHistoryCandidate("otto", candidate({ projectName: "Draek2077/otto-code" })),
    ).not.toBeNull();
  });
...
describe("rankAgentHistoryCandidates", () => {
  ...
  it("ranks the workspace name above the project name that every session shares", () => {
    const ranked = rankAgentHistoryCandidates(
      "otto",
      [
        candidate({ title: "unrelated work", projectName: "Draek2077/otto-code" }),
        candidate({ workspaceName: "otto", projectName: "Draek2077/otto-code" }),
      ],
      byUpdatedAtDesc,
    );
    expect(ranked[0].candidate.project.workspaceName).toBe("otto");
  });
