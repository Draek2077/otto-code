...
function getPidFilePath(ottoHome: string): string {
  return join(ottoHome, "otto.pid");
}
...
function createLockHeldError(lock: PidLockInfo): PidLockError {
  return new PidLockError(
    `Another Otto daemon is already running (PID ${lock.pid}, started ${lock.startedAt})`,
    lock,
  );
}
  ...
  try {
    ...
    if (raceLock) {
      throw new PidLockError(
        `Another Otto daemon is already running (PID ${raceLock.pid})`,
        raceLock,
      );
    }
...
export async function acquirePidLock(
  ...
  const pidPath = getPidFilePath(ottoHome);

  // Ensure ottoHome directory exists
  ...
  const lockInfo: PidLockInfo = {
    ...
    listen,
    heartbeat: true,
    ...(process.env.OTTO_DESKTOP_MANAGED === "1" ? { desktopManaged: true } : {}),
  };
...
export function startPidLockHeartbeat(
  ...
  const timer = setInterval(() => {
    ...
    refreshing = true;
    refreshPidLock(ottoHome, { ownerPid: options?.ownerPid })
      .catch((error) => {
        if (options?.onError) {
...
export async function getPidLockInfo(ottoHome: string): Promise<PidLockInfo | null> {
  const pidPath = getPidFilePath(ottoHome);
