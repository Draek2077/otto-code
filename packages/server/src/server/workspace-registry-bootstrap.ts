...
import { pinOttoWorktreeBranchIdentityIfMissing } from "../utils/worktree-metadata.js";
...
export async function bootstrapWorkspaceRegistries(options: {
  ...
  for (const workspace of await options.workspaceRegistry.list()) {
    ...
    try {
      pinOttoWorktreeBranchIdentityIfMissing(workspace.worktreeRoot, workspace.branch);
    } catch (error) {
      options.logger.warn(
