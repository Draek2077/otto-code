...
test("blocked setup remains readable on mixed-capability sockets sharing a session", async () => {
  await daemon.close();
  const root = await mkdtemp(path.join(os.tmpdir(), "otto-blocked-compat-"));
  const projects = path.join(root, ".otto", "projects");
  ...
  daemon = await createTestOttoDaemon({ ottoHomeRoot: root });
...
test("plugin items are gated in provider child streams, child fetches, and rewind replay", async () => {
  ...
  daemon = await createTestOttoDaemon({ isDev: true, agentClients: { mock: provider } });
