...
describe("STTManager", () => {
  ...
  it("uses OTTO_VOICE_LANGUAGE over OTTO_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses OTTO_DICTATION_LANGUAGE when OTTO_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.OTTO_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.OTTO_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.OTTO_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.OTTO_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
