...
function createCatalog(): OttoToolCatalog {
  const tool = {
    name: "echo_context",
    title: "Echo context",
...
describe("OpenCodeBridge", () => {
  test("loads packaged bundle bytes without invoking source compilation", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "otto-opencode-artifact-"));
  ...
  test("serves authenticated session context and caller-scoped tools", async () => {
    const ottoHome = await mkdtemp(path.join(tmpdir(), "otto-opencode-bridge-"));
    temporaryDirectories.push(ottoHome);
    const catalog = createCatalog();
    const bridge = new OpenCodeBridge({ ottoHome, logger: createTestLogger() });
    await bridge.start();
    bridge.setManifestCatalog(catalog);
    const release = bridge.bindSession({
      sessionId: "ses_one",
      env: {
        OTTO_AGENT_ID: "agent-one",
        OTTO_AGENT_CWD: "/workspace/one",
        CUSTOM_VALUE: "one",
      },
      tools: catalog,
    });
    ...
    try {
      ...
      await expect(
        hooks.tool.otto_echo_context.execute(
      ...
      expect(pluginError).toHaveBeenCalledWith(
        "[otto-opencode-plugin] shell.env failed",
  ...
  test("preserves user OpenCode config while installing one content-addressed plugin", async () => {
    const ottoHome = await mkdtemp(path.join(tmpdir(), "otto-opencode-bridge-config-"));
    temporaryDirectories.push(ottoHome);
    const bridge = new OpenCodeBridge({ ottoHome, logger: createTestLogger() });
    await bridge.start();
    ...
    try {
      ...
      expect(config.plugin[1]?.[0]).toMatch(/otto-[a-f0-9]{64}\.mjs$/);
    } finally {
      await bridge.close();
    }
  });
