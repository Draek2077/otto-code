...
test("real OpenCode server shares one process while shell.env stays session-scoped", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "otto-opencode-real-"));
  const firstCwd = path.join(root, "first");
  const secondCwd = path.join(root, "second");
  const logger = createTestLogger();
  const bridge = new OpenCodeBridge({ ottoHome: root, logger });
  await bridge.start();
  ...
  try {
    ...
    first = await client.createSession(
      { provider: "opencode", cwd: firstCwd, model: "opencode/big-pickle", modeId: "build" },
      {
        ...
        env: { OTTO_AGENT_ID: "real-agent-one", OTTO_AGENT_CWD: firstCwd },
        ottoTools: firstTools,
      },
    ...
    second = await client.createSession(
      { provider: "opencode", cwd: secondCwd },
      {
        ...
        env: { OTTO_AGENT_ID: "real-agent-two", OTTO_AGENT_CWD: secondCwd },
        ottoTools: secondTools,
      },
      { persistSession: false },
    );
    ...
    const [firstShell, secondShell] = await Promise.all([
      sdk.session.shell({
        ...
        directory: firstCwd,
        agent: "build",
        command: 'printf "%s|%s" "$OTTO_AGENT_ID" "$OTTO_AGENT_CWD"',
      }),
      sdk.session.shell({
    ...
    const agentResult = await first.run(
      [
        "Use the bash tool to run: env | grep -E '^(OTTO_AGENT_ID|OTTO_AGENT_CWD)='",
...
function createCallerCatalog(callerAgentId: string): OttoToolCatalog {
  const tool = {
