...
const OTTO_PI_SUBMITTED_USER_ENTRY_MARKER = "OTTO_SUBMITTED_USER_ENTRY";
...
function buildResumeStartInput(input: {
  ...
  return {
    ...
    extensionPaths: input.ottoExtension ? [input.ottoExtension.path] : undefined,
  };
}
...
function createPiOttoExtensionFile(systemPrompt?: string): PiTempFile {
	...
	export default function ottoIntegration(pi) {
	  ...
	  function emitSubmittedUserEntries(ctx) {
	    ...
	    for (let index = 0; index < submittedUserMessages.length; index += 1) {
	      ...
	      index -= 1;
	      ctx.ui.notify(
	        "${OTTO_PI_SUBMITTED_USER_ENTRY_MARKER} " +
...
export class PiRpcAgentSession implements AgentSession {
  ...
  private handleSubmittedUserEntryMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, OTTO_PI_SUBMITTED_USER_ENTRY_MARKER);
    if (!payload) {
      return false;
    }
  ...
  async createSession(
    ...
    try {
      runtimeSession = await this.runtime.startSession({
        ...
        extensionPaths: ottoExtension ? [ottoExtension.path] : undefined,
      });
    } catch (error) {
      mcpConfig?.cleanup();
