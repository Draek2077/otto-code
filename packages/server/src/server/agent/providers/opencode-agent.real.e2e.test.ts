...
const NORMAL_SENTINEL = "OTTO_NORMAL_RECOVERY_SENTINEL_7F31";
const ROOT_SENTINEL = "OTTO_ROOT_RECOVERY_SENTINEL_8C42";
const CHILD_SENTINEL = "OTTO_CHILD_RECOVERY_SENTINEL_5A19";
const PARENT_SENTINEL = "OTTO_PARENT_RECOVERY_SENTINEL_6B20";
...
async function createRealHarness() {
  const artifactDir = mkdtempSync(path.join(os.tmpdir(), "otto-opencode-recovery-"));
  const runtime = createDisposableRuntime();
  const { runtimeDir, home, ottoHome, xdgConfig, xdgData, xdgCache, xdgState, workspace } = runtime;
...
function createDisposableRuntime(
  ...
  try {
    runtimeDir = mkdtempSync(path.join(os.tmpdir(), "otto-opencode-runtime-"));
    onCreate?.(runtimeDir);
    ...
    for (const directory of [home, ottoHome, xdgConfig, xdgData, xdgCache, xdgState, workspace]) {
    ...
    return { runtimeDir, home, ottoHome, xdgConfig, xdgData, xdgCache, xdgState, workspace };
  } catch (error) {
    if (runtimeDir) {
      rmSync(runtimeDir, { recursive: true, force: true });
    }
