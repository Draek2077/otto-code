...
import {
  ...
  serializeOttoToolInputParameters,
} from "../../tools/otto-tool-serialization.js";
import type { OttoToolCatalog, OttoToolResult } from "../../tools/types.js";
...
function sendMissingCatalogResult(
  ...
  runtimeSession.sendHostToolResult(
    toOmpHostToolErrorResult(
      request.id,
      `Host tool "${request.toolName}" was called before Otto tools were registered`,
    ),
  );
}
...
class OmpHostToolRouter {
  ...
  private readonly catalog: OttoToolCatalog;
  ...
  private sendUpdate(callId: string, result: OttoToolResult): void {
...
function toOmpHostToolResult(id: string, result: OttoToolResult): OmpRpcHostToolResult {
...
function toOmpAgentToolResult(result: OttoToolResult): OmpAgentToolResult {
