...
export class OmpAgentSession implements AgentSession {
  ...
  constructor(options: OmpAgentSessionOptions) {
    ...
    this.ottoTools = options.ottoTools;
  ...
  private readonly ottoTools?: OttoToolCatalog;
  ...
  private handleSessionEvent(event: OmpAgentSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case "agent_end": {
        ...
        // so do not advertise Otto idle until it reports that transition.
...
export class OmpAgentClient implements AgentClient {
  ...
  private async configureNativeOttoTools(
    runtimeSession: OmpRuntimeSession,
    catalog: OttoToolCatalog | undefined,
  ): Promise<void> {
    if (!catalog) {
      return;
    }
  ...
  async createSession(
    ...
    try {
      await this.configureNativeOttoTools(runtimeSession, launchContext?.ottoTools);
      return new OmpAgentSession({
        ...
        ottoTools: launchContext?.ottoTools,
      });
    } catch (error) {
