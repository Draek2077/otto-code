...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Otto using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "otto-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Otto issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
