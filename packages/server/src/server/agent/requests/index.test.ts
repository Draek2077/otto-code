...
test("queued sends retain their receipt across retries and daemon reconstruction", async () => {
  const { requests, directory } = await fixture();
  let deliveries = 0;
  const input = {
    agentId: "queued-agent",
    messageId: "message-1",
    request: { prompt: "Continue next turn", delivery: "queue" },
    send: async () => {
      deliveries++;
      return { disposition: "queued" as const, queuedMessageId: "queue-entry-1" };
    },
  };
  const expected = { disposition: "queued", queuedMessageId: "queue-entry-1" };
  expect(await requests.send(input)).toEqual(expected);
  expect(await requests.send(input)).toEqual(expected);
  expect(await new AgentRequests(directory).send(input)).toEqual(expected);
  expect(deliveries).toBe(1);
  await expect(
    requests.send({ ...input, request: { ...input.request, delivery: "interrupt" } }),
