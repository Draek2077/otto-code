...
import { OttoSendResultSchema, type OttoSendResult } from "./otto-send-result.js";

const ReceiptSchema = z.object({
  fingerprint: z.string(),
  agentId: z.string(),
  state: z.enum(["pending", "completed"]),
  // COMPAT(sendReceiptResult): added in v0.9.10; old receipts lack a result.
  // Remove the optional read after the supported receipt floor passes 2027-03-13.
  sendResult: OttoSendResultSchema.optional(),
});
...
export class AgentRequests {
  ...
  async send(input: {
    ...
    messageId: string;
    request: unknown;
    send: () => Promise<void | OttoSendResult>;
    prepare?: () => Promise<void>;
  }): Promise<OttoSendResult | undefined> {
    ...
    const receipt = await readReceipt(
      path.join(this.directory, `${digest(["send", input.agentId, input.messageId])}.json`),
    );
    return receipt?.sendResult;
  }
  ...
  private execute(
    ...
    operation: {
      ...
      run: (agentId: string) => Promise<void | OttoSendResult>;
  ...
  private async executeOnce(
    ...
    let sendResult: void | OttoSendResult;
    try {
      sendResult = await operation.run(receipt.agentId);
    } catch (error) {
