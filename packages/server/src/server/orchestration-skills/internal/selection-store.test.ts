...
async function createStore() {
  const root = await mkdtemp(path.join(os.tmpdir(), "otto-agent-skills-config-"));
  roots.push(root);
...
describe("daemon agent skill selection", () => {
  ...
  it("persists a normalized custom selection under agents.skills.selection", async () => {
    const { root, store } = await createStore();
    await store.set({ mode: "custom", skills: ["otto-loop", "otto", "otto"] });
  ...
  it("does not expose selection through the generic config patch path", async () => {
    ...
    config.patch({
      skills: { selection: { mode: "custom", skills: ["otto"] } },
