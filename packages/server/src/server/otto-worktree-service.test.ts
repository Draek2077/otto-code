...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createOttoWorktree(
    {
      cwd: repoDir,
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createOttoWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createOttoWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
// Worktree restore (Unit 3): recreate a deleted Otto-owned worktree from its
...
// repoRoot=null, so deleteOttoWorktree's `git worktree remove`/`prune` is
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isOttoOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
