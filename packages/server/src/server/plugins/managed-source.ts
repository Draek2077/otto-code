...
import { z } from "zod";
import { PluginIdSchema, type PluginSourceStatusItem } from "@otto-code/protocol/messages";
...
export class ManagedPluginSources {
  ...
  constructor(ottoHome: string) {
    this.root = path.join(ottoHome, "plugins");
...
async function clone(remote: string, checkoutRoot: string): Promise<void> {
  ...
  const cloneRemote = publicRemote === remote ? remote : "https://otto.invalid/plugin.git";
  const envOverlay =
    cloneRemote === remote
      ? GIT_ENV
      : {
