...
describe("plugin manifest", () => {
  it("accepts an upstream manifest but rejects an ambiguous pair", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "plugin-manifest-alias-"));
    directories.push(directory);
    const content = JSON.stringify({ id: "upstream", requirements: { paseo: "^0.8.0" } });
    await writeFile(path.join(directory, "paseo-plugin.json"), content);
    await expect(readPluginManifest(directory)).resolves.toEqual(JSON.parse(content));
    await writeFile(path.join(directory, "otto-plugin.json"), content);
    await expect(readPluginManifest(directory)).rejects.toThrow(
      "both otto-plugin.json and paseo-plugin.json",
    );
  });
  ...
  it("reads and validates requirements before any plugin code runs", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "otto-plugin-manifest-"));
    directories.push(directory);
    const manifest = path.join(directory, "otto-plugin.json");
