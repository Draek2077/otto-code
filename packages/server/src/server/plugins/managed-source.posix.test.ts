...
async function createRepository(): Promise<string> {
  const repository = await mkdtemp(path.join(tmpdir(), "otto-plugin-git-repository-"));
  ...
  await writeFile(
    ...
    JSON.stringify({ id: "managed-example", requirements: { paseo: ">=0.8.0" } }),
  );
...
describe("managed Git plugin sources", () => {
  it("does not expose Git URL credentials when cloning fails", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "otto-plugin-git-home-"));
    roots.push(home);
