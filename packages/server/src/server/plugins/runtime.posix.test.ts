...
async function createPlugin(id: string, source: string): Promise<string> {
  ...
  await writeFile(path.join(directory, "otto-plugin.json"), JSON.stringify({ id }), "utf8");
...
export default function contribute(plugin: any) {
  ...
  // @otto/plugin name. Drop with the specifiers in plugin-sdk-specifiers.ts.
  it("loads a plugin that imports the pre-rename @otto/plugin specifier", async () => {
    const directory = await createPlugin(
      "legacy-sdk",
      `import { z } from "zod";
import { defineRpc } from "@otto/plugin/server";
