...
async function createPlugin(id: string, source: string): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "otto-plugin-service-"));
...
describe("PluginService", () => {
  it("resolves a provider icon path to sanitized inline SVG", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "otto-plugin-home-"));
    roots.push(home);
  ...
  it("prefers an existing directory and installs its selected plugin subdirectory", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ id: "local-monorepo", requirements: { paseo: ">=0.8.0" } }),
    );
    await writeFile(
  ...
  it("keeps the running commit when a Git update build command fails", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "otto-plugin-home-"));
    roots.push(home);
    const repository = await mkdtemp(path.join(tmpdir(), "otto-plugin-repository-"));
    roots.push(repository);
    await runGitCommand(["init", "-b", "main"], { cwd: repository });
    await runGitCommand(["config", "user.name", "Otto Tests"], { cwd: repository });
    await runGitCommand(["config", "user.email", "otto@example.test"], { cwd: repository });
    await writeFile(
      path.join(repository, "otto-plugin.json"),
      JSON.stringify({ id: "git-update", requirements: { paseo: ">=0.8.0" } }),
    );
    await writeFile(
  ...
  it("activates an update when the enabled plugin previously failed to start", async () => {
    ...
    await writeFile(
      path.join(repository, "otto-plugin.json"),
      JSON.stringify({ id: "failed-update", requirements: { paseo: ">=0.8.0" } }),
    );
    await writeFile(path.join(repository, "index.server.ts"), "export default () => () => {};\n");
    await runGitCommand(["add", "-A"], { cwd: repository });
    await runGitCommand(["commit", "-m", "initial"], { cwd: repository });
...
export default function contribute(plugin: unknown) {
  ...
  it("reports invalid manifests, missing entries, and startup failures", async () => {
    ...
    await writeFile(path.join(invalid, "otto-plugin.json"), JSON.stringify({}));
