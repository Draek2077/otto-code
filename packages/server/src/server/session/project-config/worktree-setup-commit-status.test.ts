...
import type { OttoConfigRaw } from "@otto-code/protocol/otto-config-schema";
...
function makeGitRepo(config?: OttoConfigRaw): string {
...
function writeConfig(repoRoot: string, config: OttoConfigRaw): void {
  writeFileSync(join(repoRoot, "otto.json"), `${JSON.stringify(config, null, 2)}\n`);
}
...
describe("worktree setup commit status", () => {
  ...
  test("resolves otto.json relative to a nested project root", async () => {
    const repoRoot = makeGitRepo();
    const projectRoot = join(repoRoot, "packages", "app");
    mkdirSync(projectRoot, { recursive: true });
    writeConfig(projectRoot, { worktree: { setup: "npm ci" } });
    execFileSync("git", ["add", "packages/app/otto.json"], { cwd: repoRoot });
