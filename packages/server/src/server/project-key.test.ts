...
describe("deriveProjectKey", () => {
  ...
  test.each([
    "https://github.com/Draek2077/otto-code.git",
  ...
  test("groups an SSH remote whether or not its default port is spelled out", () => {
    expect(derive("ssh://git@github.com:22/Draek2077/otto-code.git")).toBe(
      derive("ssh://git@github.com/Draek2077/otto-code.git"),
    );
  });
  ...
  test("keeps a repository root distinct from one of its subprojects", () => {
    ...
    const remoteUrl = "git@github.com:Draek2077/otto-code.git";
  ...
  test("uses the owner and repository from a remote", () => {
    expect(
      deriveProjectGroupingDisplayName({
        ...
        remoteUrl: "git@github.com:Draek2077/otto-code.git",
