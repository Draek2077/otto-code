...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "otto-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.OTTO_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per OTTO_HOME", () => {
  ...
  it("respects and persists OTTO_SERVER_ID override", () => {
    process.env.OTTO_SERVER_ID = "test-daemon-id";
