...
test("an empty project becomes Git without changing its identity or creating a workspace", async () => {
  const projectRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "otto-project-becomes-git-")),
  );
  const ottoHomeRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "otto-project-becomes-git-home-")),
  );
  ...
  const persistedProjects = JSON.parse(
    await readFile(path.join(daemon.ottoHome, "projects", "projects.json"), "utf8"),
