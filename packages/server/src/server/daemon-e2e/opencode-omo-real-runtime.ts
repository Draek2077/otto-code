...
export async function createOpenCodeOmoRealRuntime(): Promise<OpenCodeOmoRealRuntime> {
  ...
  const openCodeVersion = process.env.OTTO_REAL_OPENCODE_VERSION?.trim() || PINNED_OPENCODE_VERSION;
  const omoVersion = process.env.OTTO_REAL_OMO_VERSION?.trim() || PINNED_OMO_VERSION;
  ...
  if (model === NO_AUTH_MODEL) {
    await runCommand({
      command: openCodeCommand,
      args: ["run", "--model", model, "Reply with exactly: OTTO_BIG_PICKLE_PROBE_OK"],
      cwd: paths.workspace,
      env: runtimeEnv,
      artifactName: "big-pickle-probe.log",
      artifacts: paths.artifacts,
      secrets,
      requiredOutput: "OTTO_BIG_PICKLE_PROBE_OK",
    });
  }
  ...
  try {
    process.env.OTTO_HOME = path.join(paths.ottoHomeRoot, ".otto");
    ...
    daemon = await createTestOttoDaemon({
      ...
      logger,
      ottoHomeRoot: paths.ottoHomeRoot,
    ...
    return {
      ...
      close: async (passed) => {
        try {
          ...
          restoreEnvironment("OTTO_HOME", previousOttoHome);
        }
      },
    };
  } catch (error) {
...
function createRuntimePaths(): RuntimePaths {
  const root = mkdtempSync(path.join(tmpdir(), "otto-real-opencode-omo-"));
  const paths: RuntimePaths = {
    root,
    home: path.join(root, "home"),
    ottoHomeRoot: path.join(root, "otto-home"),
...
function resolveModel(openRouterApiKey: string | null): string {
  const explicitModel = process.env.OTTO_REAL_OPENCODE_MODEL?.trim();
  if (explicitModel) {
...
function buildRuntimeEnv(paths: RuntimePaths, openRouterApiKey: string | null): NodeJS.ProcessEnv {
  return {
    ...
    OTTO_HOME: path.join(paths.ottoHomeRoot, ".otto"),
