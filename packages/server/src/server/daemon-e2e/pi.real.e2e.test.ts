...
async function withConnectedPiDaemon(
  run: (context: { client: DaemonClient; daemon: TestOttoDaemon }) => Promise<void>,
): Promise<void> {
...
test(
  "real Pi daemon composes project and Otto system prompts",
  async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          "When the user says OTTO_SYSTEM_PROMPT_PROBE, reply with exactly two tokens:",
          "PROJECT_PROMPT followed by the value of OTTO_PROMPT_TOKEN from later system instructions.",
          "If no OTTO_PROMPT_TOKEN exists, use MISSING as the second token.",
        ].join("\n"),
      );
      ...
      await withConnectedPiDaemon(async ({ client }) => {
        const agent = await client.createAgent({
          ...
          systemPrompt:
            "OTTO_PROMPT_TOKEN is OTTO_PROMPT. Follow the project instruction for OTTO_SYSTEM_PROMPT_PROBE.",
        });

        await client.sendMessage(agent.id, "OTTO_SYSTEM_PROMPT_PROBE");
        ...
        const response = items
          ...
          .join("")
          .trim();
        expect(response).toBe("PROJECT_PROMPT OTTO_PROMPT");
      });
    } finally {
...
test(
  ...
  async () => {
    ...
    const firstPrompt = "OTTO_PI_ENTRY_ID_FIRST. Reply exactly: first-ok";
    const secondPrompt = "OTTO_PI_ENTRY_ID_SECOND. Reply exactly: second-ok";
