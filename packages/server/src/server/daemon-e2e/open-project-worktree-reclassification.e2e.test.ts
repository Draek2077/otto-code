...
test("openProject reclassifies an existing directory workspace into its parent git project", async () => {
  ...
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "otto-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "otto-open-project-worktree-")),
    );
    const ottoHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "otto-open-project-home-")),
    );
    ...
    const workspacesPath = path.join(ottoHome, "projects", "workspaces.json");
