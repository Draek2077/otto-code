...
describe.skipIf(isPlatform("win32"))("terminal POSIX-only", () => {
  ...
  it("reuses zsh shell integration copied from read-only source files", () => {
    const integrationSourceDir = mkdtempSync(join(tmpdir(), "otto-zsh-readonly-source-"));
    const tmpHome = mkdtempSync(join(tmpdir(), "otto-zsh-readonly-home-"));
    ...
    chmodSync(join(integrationSourceDir, "otto-integration.zsh"), 0o444);
  ...
  describe("terminal title", () => {
    it.skipIf(!hasZsh)("restores the user's ZDOTDIR through the zsh wrapper", async () => {
      ...
      writeFileSync(join(realZdotdir, ".zshenv"), "export OTTO_TEST_REAL_ZDOTDIR=1\n");
      ...
      const session = trackSession(
        await createTerminal({
          ...
          cwd: homeDir,
          command: "/bin/zsh",
          args: ["-c", 'printf \'%s\\n%s\\n\' "${ZDOTDIR-}" "${OTTO_TEST_REAL_ZDOTDIR-}"'],
          env: {
            HOME: homeDir,
            ZDOTDIR: realZdotdir,
          },
    ...
    it.skipIf(!hasZsh)("loads the user's zsh prompt when the integration dir is packaged", () => {
      ...
      temporaryDirs.push(homeDir);
      writeFileSync(join(homeDir, ".zshrc"), "PS1='OTTO_CUSTOM_PROMPT> '\n");

      const fakeAppRoot = join(homeDir, "Otto.app", "Contents", "Resources");
