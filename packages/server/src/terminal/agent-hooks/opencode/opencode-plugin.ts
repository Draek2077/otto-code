...
export const OPENCODE_PLUGIN_SOURCE = [
  ...
  "",
  "export default {",
  '  id: "otto-terminal-activity",',
  "  server() {",
  "    return {",
  "      event: async ({ event }) => {",
  "        const ottoEvent = ottoEventForV1(event);",
  "        if (ottoEvent) await runOttoHook(ottoEvent);",
  "      },",
  "    };",
  "  },",
  "  setup(ctx) {",
  ...
  "        const ottoEvent = V2_EVENTS[event.type];",
  "        if (ottoEvent) await runOttoHook(ottoEvent);",
  "      }",
  "    })().catch(() => {});",
