  ...
  it("installs POSIX and Windows hook commands idempotently", () => {
    ...
    for (const event of codexAgentHookProvider.events) {
      expect(commandHooks(config, event.event)).toEqual([
        {
          command: `if [ -n "$OTTO_TERMINAL_ID" ]; then "\${OTTO_HOOK_CLI:-otto}" hooks codex ${event.event}; fi`,
          commandWindows: `if defined OTTO_TERMINAL_ID (if defined OTTO_HOOK_CLI ("%OTTO_HOOK_CLI%" hooks codex ${event.event}) else (otto hooks codex ${event.event})) else (exit /b 0)`,
        },
      ]);
    }
