...
import { buildSelfNodeCommand } from "../server/otto-env.js";
...
describe("execCommand", () => {
  ...
  test("treats env as the replacement base and finalizes external command env", async () => {
    ...
    expect(parsePrintedEnv(result.stdout)).toEqual({
      ...
      OTTO_DESKTOP_MANAGED: null,
      OTTO_NODE_ENV: null,
      OTTO_SUPERVISED: null,
    });
  });

  test("does not inherit process.env when env replacement is supplied", async () => {
    process.env.OTTO_TEST_SHOULD_NOT_LEAK = "leaked";
    try {
      const result = await execCommand(
        process.execPath,
        [
          "-e",
          "console.log(JSON.stringify({ leaked: process.env.OTTO_TEST_SHOULD_NOT_LEAK ?? null }))",
        ],
        {
          env: {
            PATH: process.env.PATH,
      ...
      delete process.env.OTTO_TEST_SHOULD_NOT_LEAK;
    }
  });
  ...
  test("internal env mode preserves Otto-owned launcher env", async () => {
    const result = await execCommand(process.execPath, ["-e", printEnvScript], {
      envMode: "internal",
      baseEnv: {
