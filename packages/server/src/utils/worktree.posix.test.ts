...
import { getOttoWorktreeMetadataPath, readOttoWorktreeMetadata } from "./worktree-metadata.js";
import {
  getCheckoutDiff,
  getCheckoutStatus,
  listCheckoutCommits,
...
describe.skipIf(isPlatform("win32"))("worktree POSIX-only", () => {
  describe("createWorktree", () => {
    ...
    it("creates a suffixed branch when checking out a branch already checked out in the main repo", async () => {
      ...
      expect(existsSync(result.worktreePath)).toBe(true);
      expect(readOttoWorktreeMetadata(result.worktreePath)?.changeRequestLookupTarget).toEqual({
        headRef: "main-1",
    ...
    it("uses the selected local or origin ref when both exist", async () => {
      ...
      const localStatus = await getCheckoutStatus(localResult.worktreePath, { ottoHome });
      ...
      await expect(
        getCheckoutDiff(localResult.worktreePath, { mode: "base", baseRef: "main" }, { ottoHome }),
      ).resolves.toMatchObject({ diff: "" });
      expect(readFileSync(join(originResult.worktreePath, "file.txt"), "utf8")).toBe(
        "from-origin\n",
      );
      expect(
        JSON.parse(readFileSync(getOttoWorktreeMetadataPath(originResult.worktreePath), "utf8")),
    ...
    it("records the branch name when the base is on a remote other than origin", async () => {
      ...
      expect(
        JSON.parse(readFileSync(getOttoWorktreeMetadataPath(result.worktreePath), "utf8")),
      ...
      await expect(
        mergeToBase(result.worktreePath, { baseRef: "main" }, { ottoHome }),
      ).rejects.toThrow(
      ...
      await mergeFromBase(result.worktreePath, { baseRef: "main" }, { ottoHome });
      ...
      await expect(
        getCheckoutDiff(result.worktreePath, { mode: "base", baseRef: "main" }, { ottoHome }),
    ...
    it("keeps a new worktree clean when its upstream ref has a newer otto.json", async () => {
      const remoteDir = join(tempDir, "remote.git");
      ...
      writeFileSync(
        join(updaterDir, "otto.json"),
        JSON.stringify({ worktree: { setup: "echo new" } }),
      );
      execFileSync("git", ["add", "otto.json"], { cwd: updaterDir });
      execFileSync("git", ["-c", "commit.gpgsign=false", "commit", "-m", "update otto.json"], {
        cwd: updaterDir,
      });
    ...
    it("preserves a dangling otto.json symlink from the selected ref", async () => {
      const externalConfigPath = join(tempDir, "outside-otto.json");
      execFileSync("git", ["checkout", "-b", "symlink-config"], { cwd: repoDir });
      symlinkSync(externalConfigPath, join(repoDir, "otto.json"));
      execFileSync("git", ["add", "otto.json"], { cwd: repoDir });
      execFileSync("git", ["-c", "commit.gpgsign=false", "commit", "-m", "add otto.json symlink"], {
        cwd: repoDir,
      });
