...
import { OttoConfigRawSchema, OttoConfigSchema } from "@otto-code/protocol/otto-config-schema";

describe("otto config schema", () => {
  it("parses an empty config without metadata generation", () => {
    const parsed = OttoConfigSchema.parse({});
  ...
  it("parses old-style worktree and scripts config unchanged", () => {
    ...
    expect(OttoConfigSchema.parse(config)).toEqual({
      worktree: {
        setup: ["npm install"],
        teardown: ["npm run clean"],
      },
