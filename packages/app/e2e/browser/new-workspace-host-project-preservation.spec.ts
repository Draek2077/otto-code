...
const SHARED_PROJECT_NAME = "Otto";
const SHARED_REMOTE_URL = "https://github.com/Draek2077/otto-code.git";
...
const test = base.extend<{ crossHostProject: CrossHostProjectScenario }>({
  crossHostProject: async ({ page: _page }, provide) => {
    ...
    const primarySharedRepo = await createTempGitRepo("new-workspace-otto-primary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const primaryDuplicateRepo = await createTempGitRepo("new-workspace-otto-duplicate-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const secondaryFallbackRepo = await createTempGitRepo("new-workspace-fallback-");
    const secondarySharedRepo = await createTempGitRepo("new-workspace-otto-secondary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    ...
    try {
      ...
      const primarySharedProject = await createProject(primaryClient, {
        ...
        workspaceName: "Primary Otto workspace",
      });
      ...
      const primaryDuplicateProject = await createProject(primaryClient, {
        ...
        projectName: "Otto duplicate",
        workspaceName: "Duplicate Otto workspace",
      });
      ...
      const secondarySharedProject = await createProject(secondaryClient, {
        ...
        projectName: SHARED_PROJECT_NAME,
        workspaceName: "Secondary Otto workspace",
      });
      ...
      if (
        ...
        throw new Error("Expected the Otto clones to have the same equivalence key");
      }
