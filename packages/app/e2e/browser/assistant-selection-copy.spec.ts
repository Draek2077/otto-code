...
const ASSISTANT_MARKDOWN = [
  ...
  "",
  "[workspace file](file:///tmp/otto%20notes.md#L4)",
  "",
...
async function selectAssistantListItemFromMarker(
  ...
  const item = assistantMessage
    .locator(`[data-otto-markdown-tag="${listTag}"]`)
    .filter({ hasText: itemText })
    .locator(':scope > [data-otto-markdown-tag="li"]')
    .filter({ hasText: itemText });

  await item.evaluate((element, selectedEndText) => {
    const marker = element.querySelector(':scope > [data-otto-markdown-list-marker="true"]');
...
async function doubleClickAssistantMarkdownText(
  ...
  await assistantMessage
    .locator(`[data-otto-markdown-tag="${tag}"]`)
    .filter({ hasText: text })
...
test("copying an assistant selection preserves Markdown structure and links", async ({
  ...
  await page.addInitScript(() => {
    localStorage.setItem("@otto:app-settings", JSON.stringify({ uiFontFamily: "serif" }));
  });
  ...
  try {
    ...
    const inlineCode = assistantMessage
      .locator('[data-otto-markdown-tag="code"]')
    ...
    expect(clipboard.html).toContain('<a href="file:///tmp/otto%20notes.md#L4">workspace file</a>');
    ...
    await selectAssistantElement(
      page,
      '[data-otto-markdown-language="typescript"] [data-otto-markdown-tag="code"]',
    );
    await copySelection(page);
    ...
    const typescriptFence = assistantMessage.locator('[data-otto-markdown-language="typescript"]');
    // The button is opacity 0 / pointerEvents none until the fence is hovered.
    await typescriptFence.hover();
    await typescriptFence.locator("[data-otto-markdown-ignore]").click();
    ...
    const bashFence = assistantMessage.locator('[data-otto-markdown-language="bash"]');
    await bashFence.hover();
    await bashFence.locator("[data-otto-markdown-ignore]").click();
