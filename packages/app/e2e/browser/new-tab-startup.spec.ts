...
test("a workspace with an existing agent never presents New during startup", async ({ page }) => {
  ...
  try {
    await page.addInitScript(() => {
      Reflect.set(globalThis, "__ottoSawVisibleNewTab", false);
      function recordVisibleNewTab() {
        ...
        if (panel?.getClientRects().length) {
          Reflect.set(globalThis, "__ottoSawVisibleNewTab", true);
        }
      }
    ...
    expect(await page.evaluate(() => Reflect.get(globalThis, "__ottoSawVisibleNewTab"))).toBe(
      false,
    );
  } finally {
