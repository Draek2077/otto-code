    ...
    ({ accessibleName, errorText, tabTestId }) => {
      ...
      (
        window as unknown as {
          __ottoAssistantImageObservation?: typeof record;
        }
      ).__ottoAssistantImageObservation = record;
    },
    {
      accessibleName: alt,
...
async function expectNoVisibleImageInstability(page: Page): Promise<void> {
  const observation = await page.evaluate(() => {
    const owner = window as unknown as {
      __ottoAssistantImageObservation?: {
        errorSeen: boolean;
        missingSeen: boolean;
        inspect(): void;
    ...
    const record = owner.__ottoAssistantImageObservation;
    if (!record) throw new Error("Assistant image stability observation was not started");
    record.inspect();
    record.observer?.disconnect();
    delete owner.__ottoAssistantImageObservation;
