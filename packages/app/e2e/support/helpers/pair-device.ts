...
export async function prepareLocalPairingHost(
  ...
  await page.addInitScript((localServerId) => {
    (window as unknown as { ottoDesktop: unknown }).ottoDesktop = {
      platform: "darwin",
      ...
      opener: {
        openUrl: async (url: string) => {
          localStorage.setItem("@otto:e2e-opened-url", url);
        },
      },
    };
  }, daemon.serverId);
...
export async function expectRelayConsent(page: Page): Promise<void> {
  ...
  await expect(modal.getByRole("link", { name: "Read how Otto relay works" })).toBeVisible();
...
export async function expectRelayUpdateRequired(page: Page): Promise<void> {
  const modal = page.getByTestId("host-page-pair-device-card");
  await expect(modal.getByText("Update the host to enable relay from Otto Desktop.")).toBeVisible();
...
export async function openRelaySecurityDocs(page: Page): Promise<void> {
  await page.getByRole("link", { name: "Read how Otto relay works" }).click();
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("@otto:e2e-opened-url")))
    .toBe("https://otto-code.me/docs/security");
}
