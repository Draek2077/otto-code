...
export async function connectDaemonWebAppOnlyThroughRelay(
  ...
  await page.routeWebSocket(/:(6868)\b/, async (socket) => {
  ...
  await page.route("**/*", async (route) => {
    ...
    await route.fulfill({
      response,
      body: html.replace(/<script>window\.__OTTO_INITIAL_DAEMON_CONNECTION__=.*?<\/script>/, ""),
    });
  });
  await page.addInitScript(
    ({ storedHost, preferences }) => {
      localStorage.setItem("@otto:daemon-registry", JSON.stringify([storedHost]));
