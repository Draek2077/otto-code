...
export interface OttoHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveOttoHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkOttoHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<OttoHomeMetadataForkResult> {
  const sourceHome = resolveOttoHomePath(input.sourceHome);
  const targetHome = resolveOttoHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Otto metadata onto the same OTTO_HOME.");
  }
