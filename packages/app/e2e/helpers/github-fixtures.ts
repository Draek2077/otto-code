...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@otto.test"], localPath);
  git(["config", "user.name", "Otto E2E"], localPath);
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@otto.test"], basePath);
  git(["config", "user.name", "Otto E2E"], basePath);
...
export async function cloneGithubRepoDefaultBranchOnly(
  ...
  git(["config", "user.email", "e2e@otto.test"], clonePath);
  git(["config", "user.name", "Otto E2E"], clonePath);
...
export async function createLocalGithubPrFixture(): Promise<LocalGhPrFixture> {
  const fixtureRoot = await mkdtemp(path.join("/tmp", "otto-e2e-local-github-pr-"));
  ...
  const githubUrl = "https://github.com/otto-e2e/local-fixture.git";
  ...
  git(["config", "user.email", "e2e@otto.test"], checkoutPath);
  git(["config", "user.name", "Otto E2E"], checkoutPath);
