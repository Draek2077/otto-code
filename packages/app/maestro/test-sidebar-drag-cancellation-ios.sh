...
OUT_DIR="/tmp/otto-sidebar-drag-cancellation-$(date +%s)"
CLIENT_EXPORTS="$REPO_ROOT/packages/client/dist/daemon-client.js"
RELAY_EXPORTS="$REPO_ROOT/node_modules/@otto-code/relay/dist/e2ee.js"
FIXTURE_ROOT=""
...
export OTTO_MAESTRO_DAEMON_HEALTH_URL="${OTTO_MAESTRO_DAEMON_HEALTH_URL:-http://127.0.0.1:6868/api/health}"
...
if ! curl --fail --silent --show-error --max-time 3 "$OTTO_MAESTRO_DAEMON_HEALTH_URL" >/dev/null; then
  echo "Otto daemon is unavailable at $OTTO_MAESTRO_DAEMON_HEALTH_URL" >&2
  exit 1
fi

FIXTURE_ROOT="$(mktemp -d /tmp/otto-sidebar-drag-fixture-XXXXXX)"
export OTTO_MAESTRO_DRAG_A_NAME="000-otto-drag-a-$(basename "$FIXTURE_ROOT")"
export OTTO_MAESTRO_DRAG_B_NAME="001-otto-drag-b-$(basename "$FIXTURE_ROOT")"
export OTTO_MAESTRO_DRAG_Z_NAME="zzz-otto-drag-z-$(basename "$FIXTURE_ROOT")"

mkdir -p "$OUT_DIR/flows"

if [ -z "${OTTO_MAESTRO_IOS_UDID:-}" ]; then
  export OTTO_MAESTRO_IOS_UDID
  OTTO_MAESTRO_IOS_UDID="$({ xcrun simctl list devices booted -j || true; } | node -e '
    let input = "";
...
render_flow() {
  ...
  local target="$2"
  perl -0pe '
    s/^appId: sh\.otto$/appId: $ENV{OTTO_MAESTRO_APP_ID}/m;
    s/\$\{OTTO_MAESTRO_APP_ID\}/$ENV{OTTO_MAESTRO_APP_ID}/g;
    s/\$\{OTTO_MAESTRO_DIRECT_ENDPOINT\}/$ENV{OTTO_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{OTTO_MAESTRO_DRAG_A_NAME\}/$ENV{OTTO_MAESTRO_DRAG_A_NAME}/g;
    s/\$\{OTTO_MAESTRO_DRAG_B_NAME\}/$ENV{OTTO_MAESTRO_DRAG_B_NAME}/g;
    s/\$\{OTTO_MAESTRO_DRAG_Z_NAME\}/$ENV{OTTO_MAESTRO_DRAG_Z_NAME}/g;
  ' "$source" > "$target"
}

for project_name in \
  "$OTTO_MAESTRO_DRAG_A_NAME" \
  "$OTTO_MAESTRO_DRAG_B_NAME" \
  "$OTTO_MAESTRO_DRAG_Z_NAME"; do
  ...
  git -C "$project_path" config user.name "Otto Maestro"
  git -C "$project_path" config user.email "maestro@otto-code.local"
...
const client = new DaemonClient({
  url: process.env.OTTO_MAESTRO_DAEMON_WS_URL,
...
const { DaemonClient } = await import(moduleUrl);
const projectNames = [
  process.env.OTTO_MAESTRO_DRAG_A_NAME,
  process.env.OTTO_MAESTRO_DRAG_B_NAME,
  process.env.OTTO_MAESTRO_DRAG_Z_NAME,
];
const client = new DaemonClient({
  url: process.env.OTTO_MAESTRO_DAEMON_WS_URL,
...
echo "Running sidebar drag cancellation regression on $OTTO_MAESTRO_IOS_UDID"
echo "Artifacts: $OUT_DIR"
(cd "$OUT_DIR" && maestro test "$FLOW" --udid "$OTTO_MAESTRO_IOS_UDID")
