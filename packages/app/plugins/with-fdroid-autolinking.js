...
const FDROID_ABI_VERSION_CODE_BLOCK = `// Otto F-Droid single-ABI version codes
def ottoAbiVersionCodes = [
    "armeabi-v7a": 1,
    "arm64-v8a": 2,
    "x86": 3,
    "x86_64": 4,
]
def ottoArchitectures = (findProperty("reactNativeArchitectures") ?: "")
    .toString()
    .split(",")
    .collect { it.trim() }
    .findAll { !it.isEmpty() }

if (ottoArchitectures.size() == 1) {
    def ottoAbi = ottoArchitectures[0]
    def ottoAbiVersionCode = ottoAbiVersionCodes[ottoAbi]
    if (ottoAbiVersionCode == null) {
        throw new GradleException("Unsupported Otto Android ABI: " + ottoAbi)
    }
    android.defaultConfig.versionCode = android.defaultConfig.versionCode * 10 + ottoAbiVersionCode
}
`;
...
function configureFdroidAppBuildGradle(contents) {
  ...
  if (!configuredContents.includes("// Otto F-Droid single-ABI version codes")) {
