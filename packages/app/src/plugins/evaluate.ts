...
import { resolvePluginSdkSpecifier } from "@otto-code/plugin";
import * as pluginClientRuntime from "@otto-code/plugin/client";
...
export function runPluginClientBundle(
  ...
  const pluginContext: PluginClientContext & { paseo: PluginClientContext["otto"] } = {
    ...runtime,
    // Both author namespaces share this API object and the host context below.
    paseo: runtime.otto,
  ...
  const clientSdk = { ...pluginClientRuntime, usePaseo: pluginClientRuntime.useOtto, useSettings };
  const runtimeRequire = (name: string): unknown => {
    const canonical = resolvePluginSdkSpecifier(name)?.canonical ?? name;
    if (canonical === "@otto-code/plugin/client/ui") return pluginUiRuntime;
    ...
    if (canonical === "@otto-code/plugin/client") return clientSdk;
    if (canonical === "@otto-code/plugin/client/react-native") {
