...
// eslint-disable-next-line no-restricted-imports -- plugin bundles receive TanStack's real runtime, not Otto's query wrappers.
...
import { createPluginContext, type PluginRegistrationCollector } from "@otto-code/plugin/host";
...
export function evaluatePluginClientBundle(id: string, bundle: string): EvaluatedPlugin {
  ...
  const runtimeRequire = (name: string): unknown => {
    ...
    if (name === "@otto-code/plugin/server") {
