import type { AgentTimelineItem, ToolCallTimelineItem } from "@otto-code/protocol/agent-types";
...
function sourceTimelineItem(item: StreamItem): AgentTimelineItem | null {
  switch (item.kind) {
    ...
    case "action_group":
    // Presentation-only grouping runs after plugin projection. It has no canonical
    // provider row to expose to an SDK transform; preserve it if already grouped.
    case "plugin":
      return null;
  }
}
