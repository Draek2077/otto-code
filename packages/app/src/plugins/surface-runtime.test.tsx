...
function borrowFromAppProvider(otto: OttoApi): OttoApi {
  let borrowed: OttoApi | null = null;
  ...
  if (!borrowed) throw new Error("Plugin surface did not receive Otto API");
  return borrowed;
}

describe("plugin surface host runtime", () => {
  it("creates a PR worktree and agent through useOtto on the selected app host", async () => {
    ...
    const otto = borrowFromAppProvider(runtime.otto);
  ...
  it("switches all plugin calls when the selected host changes", async () => {
    ...
    await borrowFromAppProvider(second.otto).workspaces.create({
