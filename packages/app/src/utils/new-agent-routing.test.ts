...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.otto/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style otto worktree paths without checkout metadata", () => {
    expect(resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.otto\\worktrees\\feature", null)).toBe(
      "C:\\Users\\me\\repo",
    );
  });

  it("returns the main repo root for otto-owned worktrees", () => {
    const checkout = {
      isOttoOwnedWorktree: true,
      worktreeRoot: "/repo/.otto/worktrees/feature",
