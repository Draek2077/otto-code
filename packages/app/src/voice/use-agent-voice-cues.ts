...
interface CueBag {
  // Identity of the source lines; a change (personality edit) rebuilds the bag.
  signature: string;
  // Remaining lines to hand out, already shuffled. Popped from the end.
  queue: string[];
  // Last line handed out, so a fresh shuffle never repeats it first.
  last: string | null;
}
const cueBags = new Map<string, CueBag>();

// Fisher–Yates in place.
function shuffleInPlace(items: string[]): string[] {
...
export function pickAgentVoiceCue(
  ...
  const signature = lines.join("\0");
  let bag = cueBags.get(bagKey);
  if (!bag || bag.signature !== signature) {
    bag = { signature, queue: [], last: null };
    cueBags.set(bagKey, bag);
  }

  if (bag.queue.length === 0) {
    bag.queue = shuffleInPlace([...lines]);
    // Popped from the end, so the last element is what plays next. If it matches
    // the previous line, swap it with the front so we never repeat back-to-back.
    const nextUp = bag.queue[bag.queue.length - 1];
    if (nextUp === bag.last && bag.queue.length > 1) {
      [bag.queue[bag.queue.length - 1], bag.queue[0]] = [bag.queue[0]!, nextUp!];
    }
  }

  const line = bag.queue.pop() ?? null;
  if (line !== null) {
    bag.last = line;
  }
  return line;
