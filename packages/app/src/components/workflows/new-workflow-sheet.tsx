...
import { openOrchestrationGraphTab } from "@/workflow-graph/open-workflow-graph-tab";
import { buildEmptyOrchestrationGraph } from "@/workflow-graph/graph-doc";
import {
...
} from "@/components/workflows/workflow-workspace-targets";
...
const FLAVOR_OPTIONS: SegmentedControlOption<OrchestrationFlavor>[] = [
  { value: "ai", label: "AI Workflow", testID: "workflow-flavor-ai" },
  { value: "graph", label: "Graph Workflow", testID: "workflow-flavor-graph" },
];
    ...
    <View style={styles.footer}>
      ...
      {isEditingDraft ? (
        <Button
          ...
          testID="edit-workflow-save-draft"
        >
          Save Draft
        </Button>
      ) : null}
    ...
    <>
      <View ref={anchorRef} collapsable={false}>
        <Pressable
          ...
          testID="workflow-workspace-trigger"
        >
          <Text
...
function ReadOnlyProjectField({
  ...
  return (
    <>
      <View style={styles.readOnlyValue} testID="workflow-project-readonly">
    ...
    <>
      <View ref={anchorRef} collapsable={false}>
        <Pressable
          ...
          testID="workflow-project-trigger"
        >
          <Text
    ...
    <>
      <View ref={anchorRef} collapsable={false}>
        <Pressable
          ...
          testID="workflow-thinking-trigger"
        >
          <Text
            style={isPlaceholder ? styles.selectTriggerPlaceholder : styles.selectTriggerText}
            numberOfLines={1}
