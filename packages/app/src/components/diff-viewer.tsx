...
import { View, Text, Pressable, ScrollView as RNScrollView } from "react-native";
...
const DiffLineNumberWidthContext = React.createContext(0);
const DiffCodeTypographyContext =
  React.createContext<React.ComponentProps<typeof Text>["style"]>(undefined);
const DiffCodeLineHeightContext = React.createContext(0);
const DiffSyntaxTokensContext = React.createContext<
  ReadonlyMap<DiffLine, NonNullable<DiffLine["tokens"]>>
>(new Map());
const DiffReviewViewportContext = React.createContext<{
  width: number;
  gutterWidth: number;
...
const DiffReviewActionsContext = React.createContext<{
  targets: ReadonlyMap<
    DiffLine,
...
function largestDiffLineNumber(lines: readonly DiffLine[]): number {
  let largest = 1;
...
function useDiffPresentation(input: {
  diffLines: readonly DiffLine[];
  document?: DiffDocument;
