...
function destroyWebviewSelector(webview: ElementSelectorWebview, sessionToken: string): void {
  ...
  void executeWebviewJavaScript(
    webview,
    `if (window.__ottoSelector?.sessionToken === ${token}) window.__ottoSelector.destroy();`,
...
function clearWebviewSelector(webview: ElementSelectorWebview, sessionToken: string): void {
  ...
  void executeWebviewJavaScript(
    webview,
    `if (window.__ottoSelector?.sessionToken === ${token}) window.__ottoSelector.destroy(); if (window.__ottoSelectorResult?.__ottoSessionToken === ${token}) window.__ottoSelectorResult = null;`,
  ).catch(ignoreWebviewJavaScriptError);
}
    ...
    void (async () => {
      try {
        const raw = await executeWebviewJavaScript(
          webview,
          `JSON.stringify(window.__ottoSelectorResult?.__ottoSessionToken === ${token} ? window.__ottoSelectorResult : null)`,
        );
        ...
        await executeWebviewJavaScript(
          webview,
          `if (window.__ottoSelectorResult?.__ottoSessionToken === ${token}) window.__ottoSelectorResult = null;`,
        ).catch(ignoreWebviewJavaScriptError);
        const cancelled = result.__cancelled === true;
        delete result.__cancelled;
        delete result.__ottoSessionToken;
...
function buildElementSelectorScript(sessionToken: string): string {
  ...
  return `
    (function() {
      ...
      window.__ottoSelectorResult = null;
      ...
      function onClick(e) {
        ...
        var result = {
          ...
          __ottoSessionToken: sessionToken,
      ...
      function onKey(e) {
        if (e.key === 'Escape') {
          destroy();
          window.__ottoSelectorResult = { __cancelled: true, __ottoSessionToken: sessionToken };
        }
      }
      ...
      window.__ottoSelector = { destroy: destroy, sessionToken: sessionToken };
