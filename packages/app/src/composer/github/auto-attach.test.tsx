...
import type { ForgeSearchItem, ForgeSearchResponse } from "@otto-code/protocol/messages";
import { useComposerGithubAutoAttach } from "./auto-attach";

type ForgeSearchPayload = ForgeSearchResponse["payload"];

const remoteUrl = "git@github.com:acme/otto.git";
const cwd = "/repo";
...
const pr101: ForgeSearchItem = {
  ...
  number: 101,
  title: "Attach PR",
  url: "https://github.com/acme/otto/pull/101",
  state: "open",
  body: null,
  labels: [],
  baseRefName: "main",
...
const issue202: ForgeSearchItem = {
  ...
  number: 202,
  title: "Attach issue",
  url: "https://github.com/acme/otto/issues/202",
  state: "open",
  body: null,
  labels: [],
  baseRefName: null,
...
describe("useComposerGithubAutoAttach", () => {
  it("adds a matching pasted GitHub PR URL as a composer attachment", async () => {
    ...
    act(() => {
      result.current.setText("Please review https://github.com/acme/otto/pull/101");
    });
  ...
  it("ignores URLs that do not match the current remote", async () => {
    ...
    act(() => {
      result.current.setText("Other repo https://github.com/other/otto/pull/101");
    });
    await flushDebounce();
  ...
  it("does not add a second pill when the ref is already attached", async () => {
    ...
    act(() => {
      result.current.setText("Already here https://github.com/acme/otto/pull/101");
    });
    await flushDebounce();
  ...
  it("does not re-add a GitHub ref removed earlier in the same composer session", async () => {
    ...
    act(() => {
      ...
      result.current.setText("Re-pasted https://github.com/acme/otto/pull/101");
    });
    await flushDebounce();
  ...
  it("handles multiple matching URLs from one paste", async () => {
    ...
    act(() => {
      result.current.setText(
        "Refs https://github.com/acme/otto/pull/101 and https://github.com/acme/otto/issues/202",
      );
    });
    await flushDebounce();
  ...
  it("releases a removed ref without waiting for its lookup to settle", async () => {
    ...
    act(() => {
      result.current.setText("Still https://github.com/acme/otto/pull/202");
    });
    await flushDebounce();
