import { isSyntaxThemeId } from "@otto-code/highlight";
import {
...
// Type-only: erased at runtime, so this is not a module cycle. The moved pick*
// helpers keep their original Partial<AppSettings> signatures.
...
import { parseAppLanguage } from "@/i18n/locales";
import {
...
/**
 * Otto's half of the settings registry (see OttoAppSettings below).
 * use-settings/storage.ts composes this module; it never imports storage.ts back.
 */

/**
 * Every setting Otto added on top of golden's AppSettings. Paseo's interface
 * extends this, so the registry entry for a new Otto setting is a field here,
 * its default in DEFAULT_OTTO_SETTINGS below, and a pick* branch - all in this
 * Otto-owned module.
 */
export interface OttoAppSettings {
...
export function parseSettingsRecord(raw: string): Record<string, unknown> | null {
  try {
...
export function migrateLegacyThemeField(stored: Record<string, unknown>): Partial<AppSettings> {
...
export function migrateTutorialFlag(stored: Record<string, unknown>): Partial<AppSettings> {
...
export function migrateSetupWizardFlag(stored: Record<string, unknown>): Partial<AppSettings> {
...
export function pickThemeAndBehaviorSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
...
export function pickFontSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
...
export function pickAgentVoiceCueSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
...
export function pickZoomRecorderSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
...
export function pickVoicePlaybackSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
...
export function pickWorkspaceLayoutSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
...
export function pickShortcutOverlaySettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  if (
...
export function pickOnboardingSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
  if (
...
export function pickTextEffectSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
...
export function pickChatCodeSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
...
// Kept out of pickWorkspaceLayoutSettings to stay under the cyclomatic-
// complexity ceiling, mirroring pickOnboardingSettings/pickPreviewSettings.
export function pickTabLayoutSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
  if (
...
export function pickPreviewSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
  if (
...
export function pickVisualizerSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
  const result: Partial<AppSettings> = {};
...
export function pickFeatureFlagSettings(stored: Partial<AppSettings>): Partial<AppSettings> {
...
export const DEFAULT_OTTO_SETTINGS: OttoAppSettings = {
