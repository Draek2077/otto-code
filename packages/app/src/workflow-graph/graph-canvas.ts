...
export interface GraphCanvasHandle {
  ...
  /** Show or hide the Edge inspector panel that floats over the canvas. */
  setEdgeInspectorVisible(visible: boolean): void;
  destroy(): void;
}
...
export function createGraphCanvas(
  ...
  ed.zoom_enter = () => {};
  ed.start();
  // The theme class goes on AFTER start(), and this ordering is load-bearing.
  // Drawflow decides what was clicked with `target.classList[0]` - a bare index,
  // not a `contains` - and only pans the viewport when that first class reads
  // "parent-drawflow" or "drawflow". start() appends "parent-drawflow" to this
  // element, so claiming index 0 with "og-canvas" first left the precanvas as
  // the only pannable surface: it is sized 100% of the viewport but travels
  // with the pan, so dragging worked inside the un-panned starting rectangle
  // and nowhere else. Adding our class second keeps "parent-drawflow" at index
  // 0 and makes the whole canvas grabbable.
  canvasEl.classList.add("og-canvas");
  ...
  // hands on. Selecting a wire fills this panel in; deselecting drops it back
  // to its empty state. Whether the panel is on screen at all is the toolbar's
  // Edge inspector toggle, not the selection.
  ...
  edgeInspector.innerHTML =
    `<div class="og-edge-heading">Edge inspector</div>` +
    `<div class="og-edge-empty">Select an edge to set the condition that lets it deliver, and which of the upstream node's fields it carries.</div>` +
    `<div class="og-edge-body">` +
  ...
  // The toolbar toggle owns whether the panel is on screen at all; the
  // selection owns whether it has an edge to talk about. Keeping the two
  // separate is what lets the toggle mean something with nothing selected -
  // it opens the panel on its empty state instead of doing nothing visible.
  let edgeInspectorVisible = true;

  const renderEdgeInspectorVisibility = (): void => {
    edgeInspector.classList.toggle("og-hidden", !edgeInspectorVisible);
    edgeInspector.classList.toggle("og-edge-unselected", selectedEdgeKey === null);
  };
  ...
  const handle: GraphCanvasHandle = {
    ...
    setEdgeInspectorVisible(visible) {
      edgeInspectorVisible = visible;
      renderEdgeInspectorVisibility();
    },
...
/* Edge inspector - floats over the canvas until the toolbar toggle hides it. */
...
.og-edge-inspector .og-edge-heading{font-size:10.5px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;color:${theme.foregroundMuted};}
.og-edge-inspector .og-edge-body{display:flex;flex-direction:column;gap:5px;}
/* Open with nothing selected: the panel says what it is for, not nothing. */
.og-edge-inspector .og-edge-empty{display:none;font-size:11px;color:${theme.foregroundMuted};line-height:1.45;}
.og-edge-inspector.og-edge-unselected .og-edge-empty{display:block;}
.og-edge-inspector.og-edge-unselected .og-edge-body{display:none;}
/* With no edge selected the panel is a caption, not a control: it must not
   steal a click from a node that happens to sit under this corner. */
.og-edge-inspector.og-edge-unselected{pointer-events:none;}
