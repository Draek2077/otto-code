...
export class DirectorySync {
  ...
  connectionChanged(connection: DirectoryConnection): boolean {
    ...
    const subscriptions = [
      ...
      // which bucket a project lives in.
      client.on("project.updated.notification", (message) => {
        if (message.type !== "project.updated.notification" || !this.isCurrent(client, source)) {
          return;
        }
        const delta = { kind: "upsert", project: message.payload.project } as const;
