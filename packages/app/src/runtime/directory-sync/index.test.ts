...
describe("DirectorySync session readiness", () => {
  ...
  it("applies a cross-session rename notification to both name carriers", async () => {
    const serverId = "project-updated-notification";
    const { client, directory } = createDirectory(serverId);
    const store = useSessionStore.getState();
    store.initializeSession(serverId, client as unknown as DaemonClient, 1);
    store.updateSessionServerInfo(serverId, {
    ...
    client.emit({
      ...
      payload: {
        ...
        workspace: {
          ...
          projectRootPath: "/repo/renamed-project",
          workspaceDirectory: "/repo/renamed-project/workspace-1",
          projectKind: "git",
    ...
    client.emit({
      ...
      payload: {
        project: {
          ...
          projectCustomName: "New name",
          projectRootPath: "/repo/renamed-project",
          projectKind: "git",
        },
        hasActiveWorkspaces: true,
      },
    });

    const session = useSessionStore.getState().sessions[serverId];
    expect(session?.projects.get("renamed-project")).toMatchObject({
      projectDisplayName: "New name",
      projectCustomName: "New name",
    });
    expect(session?.workspaces.get("workspace-1")).toMatchObject({
      projectDisplayName: "New name",
      projectCustomName: "New name",
    });
    directory.dispose();
