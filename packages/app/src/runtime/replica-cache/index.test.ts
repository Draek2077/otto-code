...
describe("ReplicaCache", () => {
  ...
  it("round-trips explicit directory and timeline commits", async () => {
    ...
    expect(restoredDirectory.projects.get("project-1")?.projectDisplayName).toBe("Otto");
  ...
  it("treats a corrupt row as a scoped miss", async () => {
    ...
    expect(restored.projects.get("project-1")?.projectDisplayName).toBe("Otto");
...
describe("Otto outcomes through the row-store owner", () => {
  it("restores the workspace comparison base, labels, and project icon", async () => {
    const storage = new MemoryStorage();
    const writer = createCache(storage);
    const baseline = directory();
    const workspace = baseline.workspaces.get("workspace-1")!;
    workspace.gitRuntime = { currentBranch: "feature", baseRef: "main", isDirty: false };
    workspace.labels = ["review"];
    const project = baseline.projects.get("project-1")!;
    project.projectCustomIconRevision = "custom-icon-2";
    project.projectIconRevision = "icon-3";
    commitDirectory(writer, SERVER_ID, baseline);
    await writer.flush();
    const restored = await createCache(storage).readWorkspace(SERVER_ID, "workspace-1");
    expect(restored?.workspace.gitRuntime).toEqual(workspace.gitRuntime);
    expect(restored?.workspace.labels).toEqual(["review"]);
    expect(restored?.project?.projectCustomIconRevision).toBe("custom-icon-2");
    expect(restored?.project?.projectIconRevision).toBe("icon-3");
  });
  ...
  it("persists reconciled rows without caching unreconciled local presentations", async () => {
    ...
    const pending = createUserMessage({
      ...
      text: "Pending",
      timestamp: new Date("2026-07-18T08:01:00Z"),
    });
    const sent = createUserMessage({
      clientMessageId: "sent",
      messageId: "provider-sent",
      text: "Sent",
      timestamp: new Date("2026-07-18T08:02:00Z"),
      timelineCursor: { epoch: "epoch-1", seq: 12 },
    });
    writer.commitTimeline(SERVER_ID, "agent-1", { ...timeline(), items: [pending, sent] });
    await writer.flush();
    expect((await createCache(storage).readTimeline(SERVER_ID, "agent-1"))?.items).toEqual([sent]);
  });

  it("keeps a bounded display-only tail when a canonical window exceeds the row limit", async () => {
    const storage = new MemoryStorage();
    const writer = createCache(storage);
    const items = Array.from({ length: 60 }, (_, index) => ({
      ...timelineItem(String(index)),
      id: String(index),
    ...
    writer.commitTimeline(SERVER_ID, "agent-1", {
      ...timeline(),
      items,
      range: { epoch: "epoch-1", startSeq: 1, endSeq: 60 },
    });
    await writer.flush();
    const restored = await createCache(storage).readTimeline(SERVER_ID, "agent-1");
    expect(restored?.items).toEqual(items.slice(-50));
    expect(restored?.range).toBeNull();
    expect(restored?.hasOlder).toBe(false);
  });

  it("does not certify a canonical range after omitting a remote image", async () => {
    const storage = new MemoryStorage();
    const writer = createCache(storage);
    const image = createUserMessage({
      ...
      timestamp: new Date("2026-07-18T08:02:00Z"),
      timelineCursor: { epoch: "epoch-1", seq: 12 },
      remoteImages: [{ path: "attachments/image.png", mimeType: "image/png" }],
    });
    writer.commitTimeline(SERVER_ID, "agent-1", { ...timeline(), items: [image] });
    await writer.flush();
    const restored = await createCache(storage).readTimeline(SERVER_ID, "agent-1");
    expect(restored?.items).toMatchObject([{ kind: "user_message", text: "Inspect" }]);
    expect(restored?.range).toBeNull();
    expect(restored?.hasOlder).toBe(false);
  });

  it("round-trips per-turn usage without invalidating canonical coverage", async () => {
    const storage = new MemoryStorage();
    const writer = createCache(storage);
    const item = { ...timelineItem(), turnUsage: { inputTokens: 100, outputTokens: 40 } };
    writer.commitTimeline(SERVER_ID, "agent-1", { ...timeline(), items: [item] });
    await writer.flush();
    const restored = await createCache(storage).readTimeline(SERVER_ID, "agent-1");
    expect(restored?.items).toEqual([item]);
    expect(restored?.range).toEqual(timeline().range);
  });

  it("restores expandable error details on the canonical notification row", async () => {
    const storage = new MemoryStorage();
    const writer = createCache(storage);
    const item: StreamItem = {
      ...
      level: "error",
      id: "mcp-error",
      message: "2 connections unavailable",
      details: ["One failed", "Two failed"],
      timestamp: new Date("2026-07-18T08:02:00Z"),
      timelineCursor: { epoch: "epoch-1", seq: 12 },
    };
    writer.commitTimeline(SERVER_ID, "agent-1", { ...timeline(), items: [item] });
  ...
  it("evicts the least recently written host within the byte budget", async () => {
    const storage = new MemoryStorage();
    const cache = createCache(storage, 3000);
    cache.setHosts(["host-a", "host-b", "host-c"]);
    cache.commitTimeline("host-a", "agent-1", timeline("A".repeat(1000)));
    await cache.flush();
    cache.commitTimeline("host-b", "agent-1", timeline("B".repeat(1000)));
    await cache.flush();
    cache.commitTimeline("host-a", "agent-1", timeline("A".repeat(1001)));
    await cache.flush();
    cache.commitTimeline("host-c", "agent-1", timeline("C".repeat(1000)));
    await cache.flush();
    expect((await storage.readAll()).map((host) => host.serverId).sort()).toEqual([
      "host-a",
      "host-c",
    ]);
  });
