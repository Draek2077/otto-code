...
test("background membership recovery retains failure context until history is caught up", async () => {
  const world = new TimelineWorld();
  world.sync.setConnected(true);
  world.sync.replaceVisibleAgentIds("workspace", ["agent-a"]);
  ...
  restoredMembership.succeed();
  const fetch = await world.nextFetch("agent-a");
  // Stable keeps automatic retries silent until the complete operation succeeds.
  // User-requested retries expose their separate retrying state below.
  expect(world.sync.getAgentTimelineStatus("agent-a")).toBe("error");
  fetch.respond({ hasNewer: false });
  await vi.waitFor(() => expect(world.sync.getAgentTimelineStatus("agent-a")).toBe("ready"));
});
...
test("confirmed missing cached agent stops automatic polling but permits explicit recovery", async () => {
  const world = new TimelineWorld();
  world.cursors.set("agent-a", { epoch: "saved", endSeq: 12 });
  world.sync.setConnected(true);
  world.sync.replaceVisibleAgentIds("workspace", ["agent-a"]);
  (await world.nextMembership()).succeed();
  (await world.nextFetch("agent-a")).fail("Agent not found: agent-a");
  await vi.waitFor(() => expect(world.sync.getAgentTimelineStatus("agent-a")).toBe("error"));
  expect(world.sync.getAgentTimelineError("agent-a")).toBe("Agent not found: agent-a");
  expect(world.scheduledDelays()).toEqual([]);
  expect(world.cursors.get("agent-a")).toEqual({ epoch: "saved", endSeq: 12 });
  world.sync.retryVisibleAgentTimeline("agent-a");
  expect(world.sync.getAgentTimelineStatus("agent-a")).toBe("retrying");
  (await world.nextFetch("agent-a")).respond({ hasNewer: false });
  await vi.waitFor(() => expect(world.sync.getAgentTimelineStatus("agent-a")).toBe("ready"));
...
test.each(["timeline unavailable", "Agent not found: another-agent", "File not found"])(
  "%s still retries the current agent automatically",
  async (message) => {
    const world = new TimelineWorld();
    world.sync.setConnected(true);
    world.sync.replaceVisibleAgentIds("workspace", ["agent-a"]);
    (await world.nextMembership()).succeed();
    (await world.nextFetch("agent-a")).fail(message);
    await vi.waitFor(() => expect(world.sync.getAgentTimelineStatus("agent-a")).toBe("error"));
    expect(world.scheduledDelays()).toHaveLength(1);
    (await world.nextRetry())();
    (await world.nextFetch("agent-a")).respond({ hasNewer: false });
    await vi.waitFor(() => expect(world.sync.getAgentTimelineStatus("agent-a")).toBe("ready"));
    world.sync.dispose();
