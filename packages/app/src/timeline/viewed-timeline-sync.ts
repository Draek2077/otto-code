import { isConfirmedMissingTimelineAgent } from "./otto/missing-agent";
import { projectOttoPromptIndex } from "./otto-prompt-index-projection";
import {
...
function applyAuthoritativeTimelineResponse(input: {
  ...
  projectOttoPromptIndex(serverId, payload, result);
  ...
  const fetchUntilCurrent = async (
    ...
    try {
      ...
      if (catchUps.get(agentId)?.generation === generation) {
        ...
        // An authoritative missing-agent answer is not a transient outage.
        // Keep the existing error owner/manual retry path; only stop automatic polling.
        const cancelRetry = isConfirmedMissingTimelineAgent(error, agentId)
          ? undefined
          : ports.schedule(() => {
