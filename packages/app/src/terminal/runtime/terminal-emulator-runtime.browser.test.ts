...
function getBrowserTerminal(): BrowserTerminal {
  const terminal = window.__ottoTerminal as BrowserTerminal | undefined;
  if (!terminal) {
...
describe("terminal emulator runtime in a real browser", () => {
  it("passes configured scrollback to xterm", async () => {
    ...
    await waitFor({
      predicate: () => window.__ottoTerminal !== undefined,
    });

    expect(window.__ottoTerminal?.options.scrollback).toBe(42_000);
  });
  ...
  it("updates scrollback on the mounted xterm", async () => {
    ...
    const terminal = window.__ottoTerminal;

    mounted.runtime.setScrollback({ lines: 42_000 });

    expect(window.__ottoTerminal).toBe(terminal);
    expect(window.__ottoTerminal?.options.scrollback).toBe(42_000);
  });
