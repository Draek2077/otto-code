...
describe("tool call detail-level projection", () => {
  ...
  it("distinguishes reads, searches, and other tools in overview", () => {
    const calls = [
      ...
      toolCall(
        "4",
        { type: "search", query: "otto", toolName: "web_search" },
        { status: "failed" },
      ),
  ...
  it("counts Otto calls separately from other tools", () => {
    const calls = [
      ...
      toolCall(
        ...
        { name: "mcp__otto__list_worktrees" },
      ),
      toolCall("3", { type: "fetch", url: "https://otto-code.me" }),
      toolCall("4", { type: "fetch", url: "https://github.com/Draek2077" }),
    ];

    const result = project({ level: "overview", head: calls });

    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { otherToolCount: 2, ottoCallCount: 2 },
    });
  });

  it("classifies direct Brave search and Otto runtime tool names", () => {
    ...
    const calls = [
      ...
      toolCall("3", unknownDetail, { name: "otto_list_providers" }),
      toolCall("4", unknownDetail, { name: "otto_list_worktrees" }),
      toolCall("5", unknownDetail, { name: "otto_list_worktrees" }),
    ...
    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { searchCount: 3, otherToolCount: 0, ottoCallCount: 3 },
    });
  });
