...
function FilePreviewBody({
  ...
  const isMarkdownFile = isRenderedMarkdownFile(filePath);
  ...
  const highlightedLines = useMemo(() => {
    if (!preview || preview.kind !== "text" || renderKind || highlightTooLarge) {
      return null;
    }

    return highlightCode(preview.content ?? "", filePath);
  }, [highlightTooLarge, renderKind, preview, filePath]);
  ...
  const session = useMemo(
    () => ({
      write(input: { content: string; expectedModifiedAt: string; expectedRevision?: string }) {
        return client.writeFsFile({ cwd, path, ...input });
      },
    }),
    [client, cwd, path],
  );
