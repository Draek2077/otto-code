...
describe("Add Project navigation", () => {
  ...
  it("restores the GitHub destination query and active parent when reopening a repository", () => {
    const repository = {
      id: "repo-1",
      nameWithOwner: "Draek2077/otto-code",
      cloneUrl: "git@github.com:Draek2077/otto-code.git",
      description: null,
      visibility: "public",
      updatedAt: null,
    };
...
describe("Add Project options", () => {
  ...
  it("offers manual URL and protocol-specific owner/repo clone choices", () => {
    expect(buildManualGithubRepositoryChoices("git@github.com:Draek2077/otto-code.git")).toEqual([
      expect.objectContaining({
        id: "manual:git@github.com:Draek2077/otto-code.git",
        nameWithOwner: "Draek2077/otto-code",
        cloneUrl: "git@github.com:Draek2077/otto-code.git",
      }),
    ]);
    expect(buildManualGithubRepositoryChoices("Draek2077/otto-code")).toEqual([
      expect.objectContaining({ cloneProtocol: "https", cloneUrl: "Draek2077/otto-code" }),
      expect.objectContaining({ cloneProtocol: "ssh", cloneUrl: "Draek2077/otto-code" }),
    ]);
    expect(buildManualGithubRepositoryChoices("otto")).toEqual([]);
  });
  ...
  it("shows final clone paths while retaining parent paths as values", () => {
    expect(
      buildCloneLocationOptions({
        ...
        repositoryName: "otto",
        existingPaths: ["~/workspace/otto"],
      }),
    ).toEqual([
      {
        id: "~/dev",
        path: "~/dev",
        displayPath: "~/dev/otto",
