...
describe("workspace-layout-store actions", () => {
  ...
  // DEFERRED(paseoDiffTab): these arrived with the Paseo v0.2.5 merge and
  // exercise diff tabs end to end, but Otto has no registered diff panel and
  // normalizeWorkspaceTabTarget still rejects working_diff/commit_diff, so
  // openTabFocused returns null and no layout is ever written. Wiring the panel
  // needs the @/git/diff-pane restructure described in panels/register-panels.ts;
  // un-skip both in the same change. Tracked in projects/README.md.
  it.skip("persists working diff tabs while stripping commit diff tabs", () => {
    const workspaceKey = createWorkspaceKey();
    const store = workspaceLayoutStore.getState();
  ...
  it.skip("canonicalizes comparison-specific working diff tab ids from persisted layouts", () => {
