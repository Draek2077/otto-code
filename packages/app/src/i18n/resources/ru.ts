...
export const ru: TranslationResources = {
  ...
  message: {
    ...
    selectionActions: {
      ...
      explain: {
        label: "Объяснить",
        description: "Разобрать так, чтобы было легко понять.",
        prompt: "Объясни это подробнее. Расскажи, что это значит и почему это важно.",
      },
      contest: {
        label: "Оспорить",
        description: "Подвергнуть сомнению и проверить, верно ли это.",
        prompt:
          "Я хочу это оспорить. Проверь, действительно ли это верно, укажи на ошибки или неявные допущения и исправь, если это неправильно.",
      },
      research: {
        label: "Исследовать",
        description: "Копнуть глубже и подкрепить фактами.",
        prompt:
          "Исследуй это глубже. Тщательно изучи вопрос, используя код и любые доступные источники, и сообщи, какие доказательства ты нашёл.",
      },
      complete: {
        label: "Завершить",
        description: "Довести до конца и восполнить недостающее.",
        prompt:
          "Доведи это до конца. Сделай то, что описано, и доделай всё, что осталось незавершённым.",
      },
    },
    attachments: {
  ...
  workspace: {
    ...
    git: {
      ...
      commitAgent: {
        ...
        messagePersonality: "Сообщение коммита напишет профиль {{name}} ({{detail}}).",
  ...
  desktop: {
    ...
    window: {
      title: "Окно",
      trayIcon: {
        title: "Показывать значок в трее",
        hint: "Сохраняет значок Otto в системном трее, пока приложение работает",
      },
      minimizeToTray: {
  ...
  editor: {
    ...
    reloadDialog: {
      title: "Перезагрузить с диска?",
      message: "Отменить несохранённые правки и загрузить текущую версию файла с диска?",
      confirm: "Перезагрузить",
    },
    discardDialog: {
      title: "Отменить изменения?",
  ...
  settings: {
    ...
    general: {
      ...
      mountedWorkspaceLimit: {
        ...
        description:
          "Сколько workspace остаются загруженными для мгновенного переключения. Укажите хотя бы столько, между сколькими вы переключаетесь. Больше - больше памяти.",
        accessibilityLabel: "Сколько Workspace остаются загруженными",
      },
      mountedTabLimit: {
        label: "Сколько вкладок остаются загруженными",
        description:
          "Сколько вкладок на панель остаются загруженными для мгновенного переключения. Оставьте пустым, чтобы следовать этому устройству ({{auto}}). Больше - больше памяти.",
    ...
    appearance: {
      ...
      agents: {
        ...
        chatOutline: {
          ...
          hint: "Показывает браузер глав у левого края широких чатов.",
          accessibilityLabel: "Структура чата",
        },
        wrapToolCallText: {
        ...
        chatMetricsBar: {
          ...
          hint: "Тонкая строка над транскриптом с общим числом токенов, стоимостью и активностью субагентов этого чата.",
    ...
    host: {
      ...
      code: {
        ...
        enabledHint:
          "Запускает языковой сервер для каждого проекта, чтобы «Перейти к определению» находил символ под курсором. Выключено - возврат к индексу по именам.",
        solution: "Управление решениями Microsoft .NET",
        solutionHint:
          "Добавляет представление решения на вкладку «Файлы», показывая .sln или .slnx так, как его видит система сборки. Требуется .NET SDK на этом хосте.",
        csharpScope: "Загрузка проектов C#",
        csharpScopeHint:
          "Какую часть .NET-workspace загружает сервер C#. «Решение» читает единственный файл решения в корне и работает намного быстрее; «Все проекты» ничего не пропускает, но на большом репозитории может занять минуты.",
      ...
      storage: {
        ...
        hostImagesHint:
          "Скриншоты и другие изображения, созданные агентами; хранятся, чтобы прошлые сообщения могли их показать. Очистку нельзя отменить.",
        ...
        previewCacheHint:
          "Локальные копии для мгновенной загрузки изображений; их безопасно очищать, они скачаются снова. Файлы, прикреплённые к сообщениям ({{size}}), хранятся отдельно.",
        clear: "Очистить",
        ...
        maxAgeHint:
          "Сколько дней хранится неиспользуемое изображение до автоматического удаления. 0 отключает.",
        maxTotal: "Максимальный размер",
        maxTotalHint:
          "Сколько мегабайт изображений хранить на этом хосте; первыми удаляются самые старые. 0 отключает.",
      },
      workspaces: {
    ...
    providers: {
      ...
      agents: {
        ...
        maxToolRoundsHint:
          "Сколько раз модель может вызвать инструменты и прочитать результаты до конца хода. Считаются раунды, а не отдельные вызовы.",
        ...
        midSessionUpdatesHint:
          "Позволяет агенту загрузить собственный AGENTS.md подпапки при первой работе в ней. Выключено - в разговоре остаётся только то, что добавили вы и модель.",
      ...
      tools: {
        policy: {
          save: "Сохранить",
          enabled: "Включить инструменты Otto",
          description:
            "Управляет инструментами Otto при запуске или возобновлении чата. Собственные инструменты провайдера и отдельные разрешения коннекторов не меняются.",
          disabledTools: "Отключённые инструменты",
          namesHint:
            "Одно точное имя инструмента на строку, например create_agent. Пустой список ничего не блокирует. Блокировка по имени действует и на инструменты коннекторов.",
          updateHost: "Обновите хост, чтобы настроить инструменты Otto для каждого провайдера.",
          unavailable: "Подключитесь к этому хосту и повторите сохранение.",
        },
      ...
      models: {
        ...
        visibilityLabel: "Показывать {{id}} в списках моделей",
