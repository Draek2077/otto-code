...
import { OTTO_SIDEBAR_NAV_LABELS, type OttoSidebarNavId } from "./otto-sidebar-nav";

export const BUILTIN_SIDEBAR_NAV_IDS = ["new-workspace", "history", "search", "schedules"] as const;
export type BuiltinSidebarNavId = (typeof BUILTIN_SIDEBAR_NAV_IDS)[number] | OttoSidebarNavId;
...
const BUILTIN_LABEL_KEYS: Record<BuiltinSidebarNavId, string> = {
  ...
  artifacts: OTTO_SIDEBAR_NAV_LABELS.artifacts.key,
  kanban: OTTO_SIDEBAR_NAV_LABELS.kanban.key,
  runs: OTTO_SIDEBAR_NAV_LABELS.runs.key,
};
...
export function resolveSidebarNavItems(input: {
  ...
  /** The host app owns which builtins exist and their fresh visibility. */
  builtinItems?: readonly { id: BuiltinSidebarNavId; visible: boolean }[];
}): SidebarNavItem[] {
  const builtinItems =
    input.builtinItems ?? BUILTIN_SIDEBAR_NAV_IDS.map((id) => ({ id, visible: true }));
  const builtinByKey = new Map<string, BuiltinSidebarNavId>(
    builtinItems.map(({ id }) => [id, id] as const),
  );
  ...
  for (const preference of input.preferences) {
    ...
    } else if (builtinByKey.has(preference.key)) {
      const id = builtinByKey.get(preference.key)!;
      placed.add(preference.key);
      items.push({
        kind: "builtin",
        key: id,
  ...
  for (const { id, visible } of builtinItems) {
    if (placed.has(id)) continue;
    items.push({ kind: "builtin", key: id, id, visible });
  }
