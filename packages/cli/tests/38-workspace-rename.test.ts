...
try {
  ...
  const renamed = await ctx.otto(["workspace", "rename", workspaceId, "Auth rework", "--json"]);
  ...
  const renamedList = await ctx.otto(["workspace", "ls", "--json"]);
