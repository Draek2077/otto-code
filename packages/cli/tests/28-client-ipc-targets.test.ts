...

{
  ...
  const target = resolveDaemonTarget("unix:///tmp/otto.sock");
  assert.deepStrictEqual(target, {
    type: "ipc",
    url: "ws+unix:///tmp/otto.sock:/ws",
    socketPath: "/tmp/otto.sock",
  });
...

{
  ...
  const target = resolveDaemonTarget("pipe://\\\\.\\pipe\\otto-managed-test");
  assert.deepStrictEqual(target, {
    type: "ipc",
    url: "ws://localhost/ws",
    socketPath: "\\\\.\\pipe\\otto-managed-test",
  });
...

{
  ...
  assert.strictEqual(normalizeDaemonHost("/tmp/otto.sock"), "unix:///tmp/otto.sock");
...

{
  ...
  assert.strictEqual(normalizeDaemonHost("C:\\Users\\foo\\.otto\\otto.sock"), null);
...

{
  ...
  const ottoHome = mkdtempSync(path.join(os.tmpdir(), "otto-client-targets-"));
  try {
    mkdirSync(ottoHome, { recursive: true });
    writeFileSync(
      path.join(ottoHome, "otto.pid"),
      JSON.stringify({ pid: process.pid, listen: "/tmp/otto-from-pid.sock" }),
    );
    assert.deepStrictEqual(resolveDefaultDaemonHosts({ OTTO_HOME: ottoHome }), [
      "unix:///tmp/otto-from-pid.sock",
      "localhost:6868",
    ]);
    const previousHome = process.env.OTTO_HOME;
    const previousHost = process.env.OTTO_HOST;
    process.env.OTTO_HOME = ottoHome;
    delete process.env.OTTO_HOST;
    assert.strictEqual(getDaemonHost(), "unix:///tmp/otto-from-pid.sock");
    if (previousHome === undefined) delete process.env.OTTO_HOME;
    else process.env.OTTO_HOME = previousHome;
    if (previousHost === undefined) delete process.env.OTTO_HOST;
    else process.env.OTTO_HOST = previousHost;
  } finally {
...

{
  ...
  const ottoHome = mkdtempSync(path.join(os.tmpdir(), "otto-client-targets-tcp-"));
  try {
    assert.deepStrictEqual(
      resolveDefaultDaemonHosts({
        OTTO_HOME: ottoHome,
        OTTO_LISTEN: "127.0.0.1:7777",
      }),
      ["127.0.0.1:7777", "localhost:6868"],
    );
  } finally {
    rmSync(ottoHome, { recursive: true, force: true });
  }
...

{
  ...
  const ottoHome = mkdtempSync(path.join(os.tmpdir(), "otto-client-targets-order-"));
  try {
    mkdirSync(ottoHome, { recursive: true });
    writeFileSync(
      path.join(ottoHome, "otto.pid"),
      JSON.stringify({ pid: process.pid, listen: "/tmp/otto-priority.sock" }),
    );
    assert.deepStrictEqual(
      resolveDefaultDaemonHosts({
        OTTO_HOME: ottoHome,
      ...
      ["unix:///tmp/otto-priority.sock", "127.0.0.1:7777", "localhost:6868"],
    );
  } finally {
    rmSync(ottoHome, { recursive: true, force: true });
  }
...

{
  ...
  const previousEnv = process.env.OTTO_PASSWORD;
  try {
    delete process.env.OTTO_PASSWORD;
    assert.strictEqual(
    ...
    process.env.OTTO_PASSWORD = "env-secret";
    assert.strictEqual(
      resolveDaemonPassword("tcp://example.com:6868?ssl=true&password=query-secret"),
      "query-secret",
    ...
    if (previousEnv === undefined) {
      ...
      process.env.OTTO_PASSWORD = previousEnv;
    }
  }
