...
try {
  // Test 1: permit --help shows subcommands
  {
    ...
    const result = await $`npx otto permit --help`.nothrow();
  ...
  // Test 2: permit ls --help shows options
  {
    ...
    const result = await $`npx otto permit ls --help`.nothrow();
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`OTTO_HOST=localhost:${port} OTTO_HOME=${ottoHome} npx otto permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`OTTO_HOST=localhost:${port} OTTO_HOME=${ottoHome} npx otto permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`OTTO_HOST=localhost:${port} OTTO_HOME=${ottoHome} npx otto -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
