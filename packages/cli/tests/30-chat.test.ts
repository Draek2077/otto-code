...
try {
...

  {
    ...
    const posted = await ctx.otto(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { OTTO_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.otto(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.otto(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.otto(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.otto(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.otto(["chat", "delete", "coord-room"]);
