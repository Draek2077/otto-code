...
async function waitForLoopInList(
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    const listed = await ctx.otto(["loop", "ls", "--json"]);
...
try {
  {
    ...
    const listed = await ctx.otto(["schedule", "ls", "--json"]);
    assert.strictEqual(listed.exitCode, 0, listed.stderr);
    const listedJson = JSON.parse(listed.stdout);
    assert(Array.isArray(listedJson), listed.stdout);
    assert(
    ...
    const inspected = await ctx.otto(["schedule", "inspect", createdJson.id, "--json"]);
...

  {
    ...
    async function pollStatus(attempt: number): Promise<string> {
      ...
      const inspect = await ctx.otto(["loop", "inspect", runJson.id, "--json"]);
    ...
    const logs = await ctx.otto(["loop", "logs", runJson.id], { timeout: 15000 });
    assert.strictEqual(logs.exitCode, 0, logs.stderr);
    assert(logs.stdout.includes("verify-check"), logs.stdout);

    const stopped = await ctx.otto(["loop", "stop", runJson.id, "--json"]);
  ...
  await ctx.stop();
  await rm(ctx.ottoHome, { recursive: true, force: true });
