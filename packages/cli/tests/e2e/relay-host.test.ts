...
import { buildRelayWebSocketUrl } from "@otto-code/protocol/daemon-endpoints";
import { parseConnectionOfferFromUrl } from "@otto-code/protocol/connection-offer";
import { generateLocalPairingOffer } from "@otto-code/server";
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      timeout: 60_000,
      env: {
        OTTO_RELAY_ENABLED: "true",
        OTTO_RELAY_ENDPOINT: relayEndpoint,
        OTTO_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });

    const offer = await generateLocalPairingOffer({
      ottoHome: ctx.ottoHome,
      relayEnabled: true,
      relayEndpoint,
  ...
  it("runs `otto --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    if (!ctx) throw new Error("test context not initialized");

    const direct = await ctx.otto(["ls", "--json"]);
    ...
    const relay = await ctx.otto(["ls", "--json", "--host", offerUrl], {
      timeout: 30_000,
      env: { OTTO_HOST: offerUrl },
    });
