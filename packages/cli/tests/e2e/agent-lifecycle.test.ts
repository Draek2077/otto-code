 ...
 * 2. Create agent with `otto run "say hello" --provider claude`
 * 3. List agents with `otto ls`
 * 4. Wait for agent with `otto wait <id>`
 * 5. Inspect agent with `otto inspect <id>`
 * 6. Stop agent with `otto stop <id>` and verify it remains inspectable
 * 7. Delete agent with `otto delete <id>`
 * 8. Cleanup: stop daemon, remove temp dirs
 *
 * CRITICAL RULES:
 * - NEVER use port 6868 (user's running daemon)
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`OTTO_HOME: ${ctx.ottoHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.otto(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.otto(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.otto(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.otto(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.otto(["stop", agentId]);
  ...
  const inspectResult = await ctx.otto(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.otto(["delete", agentId]);
