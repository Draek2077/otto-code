...
async function main(): Promise<void> {
  ...
  try {
    ...
    const init = await context.otto(["plugin", "init", scaffold, "--json"]);
    assert.equal(init.exitCode, 0, init.stderr);
    const manifestPath = path.join(scaffold, "otto-plugin.json");
    const manifest = await readPluginManifest(scaffold);
    assert.deepEqual(manifest.requirements, { paseo: `>=${PASEO_PLUGIN_API_VERSION}` });
    await writeFile(
      path.join(directory, "otto-plugin.json"),
      JSON.stringify({ id: "cli-e2e", requirements: { paseo: `>=${PASEO_PLUGIN_API_VERSION}` } }),
    );
    await writeFile(path.join(directory, "index.server.ts"), pluginSource);

    const install = await context.otto(["plugin", "install", directory, "--json"]);
    ...
    const incompatibleInstall = await context.otto(["plugin", "install", scaffold, "--json"]);
    assert.equal(incompatibleInstall.exitCode, 1);
    assert.match(incompatibleInstall.stderr, /requires Paseo >=999.0.0/);
    const afterRejection = await context.otto(["plugin", "ls", "--json"]);
    ...
    const scaffoldInstall = await context.otto(["plugin", "install", scaffold, "--json"]);
    ...
    await writeFile(
      path.join(gitDirectory, "otto-plugin.json"),
      JSON.stringify({
        id: "git-cli-e2e",
    ...
    const gitInstall = await context.otto([
      "plugin",
      "add",
    ...
    const status = await context.otto(["plugin", "status", "git-cli-e2e", "--json"]);
    ...
    const update = await context.otto(["plugin", "update", "git-cli-e2e", "--json"]);
    ...
    const incompatibleUpdate = await context.otto(["plugin", "update", "git-cli-e2e", "--json"]);
    ...
    const retained = await context.otto(["plugin", "ls", "git-cli-e2e", "--json"]);
    ...
    const incompatibleAdd = await context.otto([
      "plugin",
      "add",
      pathToFileURL(gitDirectory).href,
      "--id",
