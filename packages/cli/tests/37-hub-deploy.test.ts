...
const cwd = await mkdtemp(path.join(tmpdir(), "otto-hub-installed-"));
...
try {
  ...
  const validate = await runLocalOtto(
    [
      "hub",
      "deploy",
      "-p",
  ...
  const install = await runLocalOtto(
