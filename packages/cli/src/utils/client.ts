...
import { parseSshTransportUri } from "@otto-code/protocol/ssh-transport";
import { DaemonClient, type WebSocketLike } from "@otto-code/client/internal/daemon-client";
...
export function buildDaemonConnectionCommandError(options: {
  ...
  return {
    ...
    details: host.trim().startsWith("ssh://")
      ? "Start the Otto daemon on the SSH host; SSH transport does not install or start it."
      : "Start the daemon with: otto daemon start",
  };
}
...
function readPidSocketTarget(ottoHome: string): string | null {
...
function resolveConfiguredIpcDaemonHost(env: NodeJS.ProcessEnv, ottoHome: string): string | null {
...
function resolveConfiguredTcpDaemonHost(env: NodeJS.ProcessEnv, ottoHome: string): string | null {
...
export function getExplicitDaemonHost(
  ...
  const explicitHost = host ?? env.OTTO_HOST;
