...
export async function runHubGuidedSetup(
  ...
  if (
    ...
    !(await requiredConfirm(environment, "Replace the existing .otto/ Hub bundle?", false))
  ) {
  ...
  if (deploy) {
    ...
    reportMessage(environment, `Skipped deployment. Run: otto hub deploy -p ${project.slug}`);
  }
...
export async function continueHubGuidedSetup(
  ...
  if (!(await requiredConfirm(environment, "Connect this daemon to this Hub?", true))) {
    reportMessage(
      environment,
      `Skipped daemon connection. Run: ${hubLoginResumeCommand("connect", origin)}; then otto hub init`,
    );
    return;
  }
...
async function chooseProject(
  ...
  if (resolution.kind === "none") {
    throw new HubInitCancelledError(
      `No Hub projects exist yet. Create one at ${new URL("/projects/new", origin).toString()}, then run otto hub init again.`,
    );
  }
