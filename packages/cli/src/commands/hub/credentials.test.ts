...
describe("Hub CLI credentials", () => {
  it("stores multiple normalized origins privately and selects the latest login", () => {
    ...
    const store = new PrivateHubCredentialStore({ OTTO_HOME: home });
  ...
  it("logs out only the active human credential and preserves other origins", () => {
    const store = new PrivateHubCredentialStore({ OTTO_HOME: temporaryHome() });
    store.save({ origin: "https://first.example.com", credential: "first-secret" });
  ...
  it("rejects malformed storage without exposing persisted secrets", () => {
    ...
    assert.throws(
      () => new PrivateHubCredentialStore({ OTTO_HOME: home }).active(),
      (error) => {
  ...
  it("resolves origin from explicit, environment, active login, then hosted default", () => {
    ...
    assert.equal(
      resolveHubOrigin({
        ...
        env: { OTTO_HUB_URL: "https://env.example.com" },
        credentials: store,
      }),
    ...
    assert.equal(
      resolveHubOrigin({
        options: {},
        env: {},
        credentials: new PrivateHubCredentialStore({ OTTO_HOME: temporaryHome() }),
      }),
      DEFAULT_HUB_ORIGIN,
    );
  });
  ...
  it("resolves credential from explicit, environment, then exact-origin stored login", () => {
    ...
    assert.equal(
      resolveHubCredential({
        ...
        env: { OTTO_HUB_API_KEY: "env-secret" },
        credentials: store,
      }),
      "explicit-secret",
    );
...
function temporaryHome(): string {
  const directory = mkdtempSync(path.join(tmpdir(), "otto-hub-credentials-"));
