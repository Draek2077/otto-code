...
describe("Hub commands", () => {
  ...
  it("login stores the durable credential and marks its normalized origin active", async () => {
    ...
    assert.deepEqual(credentials.active(), {
      ...
      credential: "otto_cli_prefix_durable-secret",
    });
  ...
  it("login without an origin uses the hosted default and reports it before authorization", async () => {
      ...
      {},
      {
        ...
        flow: {
          authorize: async (origin) => {
            ...
            return "otto_cli_prefix_durable-secret";
          },
        },
    ...
    assert.deepEqual(events, [
      "progress:Logging in to https://hub.otto-code.me",
      "authorize:https://hub.otto-code.me",
      "progress:Logged in",
    ]);
    assert.equal(result.data.origin, "https://hub.otto-code.me");
  });
  ...
  it("connect without authority reports the hosted destination and contacts nothing", async () => {
    const progress: string[] = [];
    const credentials = new MemoryCredentials();
    const daemon = new FakeDaemonConnection(new FakeDaemon("https://hub.otto-code.me"));
    let hubRequests = 0;
    ...
    await assert.rejects(
      ...
      ),
      {
        ...
        message:
          "No stored Hub login matches https://hub.otto-code.me. Run `otto hub login https://hub.otto-code.me`, pass --api-key <secret>, or set OTTO_HUB_API_KEY.",
      },
    );

    assert.deepEqual(progress, ["Connecting this daemon to https://hub.otto-code.me"]);
  ...
  it("projects uses explicit API-key authority and renders auditable JSON output", async () => {
    ...
    const result = await runHubProjects(
      ...
      {
        env: { OTTO_HUB_URL: "https://env.test", OTTO_HUB_API_KEY: "env-secret" },
        credentials,
        hub: {
