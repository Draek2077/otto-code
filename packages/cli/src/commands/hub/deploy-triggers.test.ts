...
describe("Hub trigger deployment discovery", () => {
  it("discovers self-contained triggers in deterministic order", async () => {
    ...
    await mkdir(path.join(cwd, ".otto", "triggers"), { recursive: true });
    await writeFile(path.join(cwd, ".otto", "triggers", "z.yml"), "name: z\n");
    await writeFile(path.join(cwd, ".otto", "triggers", "a.yml"), "name: a\n");

    await expect(discoverHubTriggers(cwd)).resolves.toEqual([
      { path: ".otto/triggers/a.yml", yaml: "name: a\n" },
      { path: ".otto/triggers/z.yml", yaml: "name: z\n" },
    ]);
  });

  it("rejects unsupported and unsafe trigger paths", async () => {
    const cwd = await temporaryDirectory();
    const directory = path.join(cwd, ".otto", "triggers");
  ...
  it("directs legacy bundles to the explicit project deployment path", async () => {
    ...
    await writeFile(path.join(cwd, ".otto", "hub.yml"), "environments: {}\n");

    await expect(discoverHubTriggers(cwd)).rejects.toMatchObject({
      code: "HUB_PROJECT_REQUIRED",
      message:
        "This directory contains a legacy .otto/hub.yml bundle. Pass --project <slug> to deploy it.",
    });
  });
});
