...
const SDK_DECLARATIONS = `declare module "@otto-code/plugin/server" {
...
declare module "@otto-code/plugin" {
...
const ENTRY = `import type { PluginContext } from "@otto-code/plugin";
...
const CLIENT_SURFACE = `import type { PluginSurfaceProps } from "@otto-code/plugin";
...
export async function scaffoldPluginDirectory(
  ...
  const files = new Map<string, string>([
    ["otto-plugin.json", `${JSON.stringify({ id }, null, 2)}\n`],
    ["package.json", `${JSON.stringify(packageJson, null, 2)}\n`],
    ["tsconfig.json", `${JSON.stringify(TSCONFIG, null, 2)}\n`],
    ["otto-plugin.d.ts", SDK_DECLARATIONS],
    ["index.ts", ENTRY],
