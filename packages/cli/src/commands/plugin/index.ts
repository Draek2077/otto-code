...
} from "@otto-code/protocol/plugin-source-reference";
...
async function install(
  ...
  process.stderr.write(
    "Trusting plugin code: server code and Git build commands run unsandboxed on the daemon host; client code runs inside Otto. Dependencies and future updates are part of the codebase you trust.\n",
  );
...
export function createPluginCommand(): Command {
  ...
  addJsonAndDaemonHostOptions(
    plugin
      ...
      .option("--id <id>", "Runtime plugin ID (defaults to otto-plugin.json id)")
